...
static void MD5Transform(ULONG buf[4], BYTE input[64]);
static void Encode(BYTE *, ULONG *, UINT);
static void Decode(ULONG *, BYTE *, UINT);
...
void MD5Final (BYTE digest[16], MD5_CTX *context)
{
  ...
  memset((PUSTR)context, 0, sizeof(*context));
}
...
static void MD5Transform(ULONG state[4], BYTE block[64])
{
 ...
 /* Round 2 */
  ...
  /* Zeroize sensitive information. */
  memset((PUSTR)x, 0, sizeof(x));
}

/* 
	Encodes input (ULONG) into output (BYTE). Assumes len is
	a multiple of 4.
*/
static void Encode(BYTE *output, ULONG *input, UINT len)
{
  UINT i, j;
...
/* 
	Decodes input (BYTE) into output (ULONG). Assumes len is a multiple of 4.
*/
static void Decode(ULONG *output, BYTE *input, UINT len)
{
  UINT i, j;
