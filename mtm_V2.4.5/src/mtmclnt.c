...
mtm_client_single_msg(void)
{
	...
	*/
	...
	{
		/*
		*	If Server of this type server type is active, send msg
		*/
	...
	fflush(stdout);
			...
			*/
			...
			/* if mtm_stats_on is TRUE, then set the time-in field in mtm header 
			*  to the current time in microseconds
			*/
				...
				for(j=0;j<MTMMaxClient;j++)
				{
					if(pHeader->sd == MTMRoutingTbl[j].sd)
					{
						...
						(void)memcpy(stbl_entry->cl_tbl[i].ip_address,
									...
									IP_ADDR_LEN);
						stbl_entry->cl_tbl[i].time_connected =
							 		MTMRoutingTbl[j].time_connected;
						(void)mtm_update_jrnl(	REQST_MSG,
								pHeader->srv_type,
