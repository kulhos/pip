...
VALIDFN(fn,scatbl)	; Check validity of selected function
	...
	N scatbl3 S scatbl3=$G(^SCATBL(1,fn,%UCLS))
...
XKILL(x)	; Save system and selected application variables
	...
	N argv
	;
	S argv="("_$$VSAV_$get(x)_",%BLK,%CACHE,%cmmd,DISPEXIT,DRVVARS,%DUP,EXIT,%ID,%JRNL,%MSGID,%MSGS,ER,RM,SYSVARS)"
	;
...
SYSVAR(sysvar)	;
	...
	N list N x N y
	;
	N cuvar,vop1,vop2,vop3,vop4,vop5,vop6,vop7 S cuvar=$$vDb25()
	 S cuvar=$G(^CUVAR(2))
	 S vop4=$G(^CUVAR("CO"))
	 S vop6=$G(^CUVAR("ODP"))
	 S vop2=$G(^CUVAR("%MCP"))
...
vSIG()	;
	Q "60876^57178^Dan Russell^26550" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vDb24(v1)	;	voXN = Db.getRecord(CTBLUDFN,,0)
	...
	Q ctbludfn
	;
vDb25()	;	voXN = Db.getRecord(CUVAR,,0)
	;
	I '$D(^CUVAR)
...
vFetch2()	;
	...
	S rs=$S(vos3=$$BYTECHAR^SQLUTL(254):"",1:vos3)_$C(9)_$P(vos4,"|",1)_$C(9)_$P(vos4,"|",2)
	;
	Q 1
	;
...
vFetch3()	;
	...
	S rs=$S(vos3=$$BYTECHAR^SQLUTL(254):"",1:vos3)_$C(9)_$P(vos4,"|",1)
	;
	Q 1
