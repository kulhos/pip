TBXUPGU	;Private;TBX upgrade utilities 
	...
	; Revision History:
	; 01/07/2004	Lik Kwan
	;		Changed to use absolute path when removing routines.
	;
	; 06/02/2003	Lik Kwan
	;		Include files obsoleted in target version.		
	;
	; 05/05/2003	Lik Kwan
	;		Modified GETCONT to ignore only folder name is "vms" or "unix"
	;		Modified DQLIST,COMPARE,OBSO2 section not to use folder.
	;
	;-----------------------------------------------------------------------
	;
	D WARN
	...
	;
	S HDG="Profile upgrade utilities"
	S ER=0,RM=""
	K ^TMP($J),^TMP1($J)
	;
	;
	...
	S ISVMS=$ZV["VMS"
	;
	S GENLIST("111")="Profile97"
	S GENLIST("119")="Profile99"
	S GENLIST("121")="Profile01"
	;
	S %TAB("INSTDIR")=".INSTDIR/REQ/DES=Build contents/TYP=T/LEN=100/XPP=D CHKFILE^TBXUPGU(X)"
	S %TAB("TGGEN")=".TGGEN/REQ/DES=Target Generation/TYP=T/LEN=10/TBL=GENLIST("
	S %READ="@HDG/CEN/REV,,,INSTDIR,TGGEN"
	D ^UTLREAD
	I VFMQ="Q" Q
	;
	...
	;
	D LOGF,DOBSO(INSTDIR,TGGEN,ISVMS,ISNFS),logmsg($G(RM))
	;
	c zlogf
	Q 
	;
	;----------------------------------------------------------------------
DOBSO(INSTDIR,TGGEN,ISVMS,ISNFS)	; element obsolete entry point
	;----------------------------------------------------------------------
	;
	...
	I +RELST=0 d logmsg($P(RELST,%,2)) S RM="Invalid release number found in release.dat" Q
	;
	S REL=$P(RELST,%,1),CURPAT=$P(RELST,%,2)
	I REL>TGGEN	S RM="Lower generation is entered." Q
	;
	D GETOBS(TGGEN,REL,CURPAT)				; build obsolete data from OBSDATA global
	;
	I $D(^TMP($J)) D OBSO1
	;
...
GETPAT();	Get last patch release installed from release.dat file 
	;-----------------------------------------------------------------------
	;	
	N IO,X,HEADER,REL,LINE,PAT,CURPAT
	...
	;
	;-----------------------------------------------------------------------
GETOBS(TGGEN,REL,LVL)	; Build obsoleted data from OBSDATA.TXT
	;-----------------------------------------------------------------------
	;
	N PATLVL,ELEM,TYPE,TMPPAT,GEN
	S PATLVL="",ELEM=""
	S GEN=REL-1
	F  S GEN=$O(^OBSDATA(GEN)) Q:(GEN="")!(GEN>TGGEN)  D
	.	I GEN=REL S PATLVL=LVL
	.	E  S PATLVL=""
	. 	F  S PATLVL=$O(^OBSDATA(GEN,PATLVL)) Q:PATLVL=""  D
	..		F  S ELEM=$O(^OBSDATA(GEN,PATLVL,ELEM)) Q:ELEM=""  D
	...			S TYPE=$P(ELEM,".",2)
...
WARN	; Preach about the use of backups, screen captures use of caution
	...
	S LINE5="Refer to the Profile Software Installation Instuctions for"
