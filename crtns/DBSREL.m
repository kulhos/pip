...
DBSREL ; 
 ...
 Q 
 ;
VALIDATE(TABLES,LOOP,RM) ; Error message /MECH=REFNAM:W
 ;
 ...
 .	I '($D(LOOP(-1,TBL))#2) D
 ..		;
 ..		N dbtbl1,vop1,vop2,vop3,vop4,vop5 S vop1="SYSDEV",vop2=TBL,dbtbl1=$$vRCgetRecord1Opt^RecordDBTBL1("SYSDEV",TBL,0,.vop3)
 ...
 I ER Q 1
 ;
 N tblrecp S tblrecp=$$getPslTbl^UCXDD(TBLS(1),0)
 ;
 S PKEYS=$P(tblrecp,"|",3)
 ;
 ; Only a single table
 ...
 Q ER
 ;
COMBOS(POS,CNT,COMBO,VALIDREL,TBLS) ; Error message, if any /NOREQ/MECH=REFNAM:W
 ;
 N I
 ;
 ...
 Q 
 ;
CHKCOMBO(COMBO,TBLS,RM) ; Error message, if any /MECH=REFNAM:W
 ;
 ...
 Q isValid
 ;
REL(TBLA,TBLB,RM) ; Error message, if any /NOREQ/MECH=REFNAM:W
 ;
 N isValid S isValid=1
 N I
 N BKEYS
 ;
 N tblrecb S tblrecb=$$getPslTbl^UCXDD(TBLB,0)
 ;
 S BKEYS=$P(tblrecb,"|",3)
 ...
 Q isValid
 ;
LINK(TBLA,TBLB,LEVA,LOOP,PKEYS) ; Primary table keys
 ;
 N I
 N AKEYS N BKEYS
 ;
 N tblreca S tblreca=$$getPslTbl^UCXDD(TBLA,0)
 N tblrecb S tblrecb=$$getPslTbl^UCXDD(TBLB,0)
 ;
 S AKEYS=$P(tblreca,"|",3)
 S BKEYS=$P(tblrecb,"|",3)
