 ; 
 ; **** Routine compiled from DATA-QWIK Report DBSFKLST1 ****
 ; 
 ; 02/24/2010 18:38 - pip
 ; 
R00S044 ; DBSFKLST1 - List Foreign Key Defn by Foreign Key
 ...
 ;
 S RID="DBSFKLST1"
 S RN="List Foreign Key Defn by Foreign Key"
 I $get(IO)="" S IO=$I
 ;
 D INIT^%ZM()
 ;
 S %TAB("IO")=$$IO^SCATAB
 S %TAB("VIN1")="|255||[DBTBL1F]TBLREF|[DBTBL1]:NOVAL||D EXT^DBSQRY||T|Foreign Filename|||||"
 ;
...
V0 ; External report entry point
 ...
 N VWHERE
 N %TIM N CONAM N RID N RN N VL N VLOF N VRF N VSTATS N vCOL N vHDG N vc1 N vc2 N vc3 N vc4 N vc5 N vc6 N vc7 N vc8 N vovc1 N vovc2 N vovc3 N vovc4 N vrundate N vsysdate N zref
 ;
 N cuvar S cuvar=$$vRCgetRecord0Opt^RecordCUVAR(0,"")
  S cuvar=$G(^CUVAR("BANNER"))
 ;
 S CONAM="PIP Version 0.2"
 S ER=0 S RID="DBSFKLST1" S RN="List Foreign Key Defn by Foreign Key"
 S VL=""
 ;
 ...
 ..		N CHRSET S CHRSET=$$^UCIOENCD("Report","DBSFKLST1","V0","*")
 ...
 D INIT^%ZM()
 ;
 S vCOL="[DBTBL1]DES#23#40,[DBTBL1F]FID#67#35,[DBTBL1F]UPD#114#1\[DBTBL1F]DEL#114#1"
 ;
 ...
 .	I VIN1'="ALL" S DQQRY(1)="[DBTBL1F]TBLREF "_VIN1 S SEQ=SEQ+1
 ...
 .	N VBNRINFO
 .	;
 .	S VBNRINFO("PROMPTS",1)="WC2|"_"Foreign Filename"_"|VIN1|"_$get(VIN1)
 .	;
 .	D
 ..		N SEQ
 ..		N VALUE N VAR N X
 ...
 ..		Q 
 .	;
 .	S VBNRINFO("DESC")="List Foreign Key Defn by Foreign Key"
 .	S VBNRINFO("PGM")="R00S044"
 .	S VBNRINFO("RID")="DBSFKLST1"
 .	S VBNRINFO("TABLES")="DBTBL1F,DBTBL1"
 .	;
 .	S VBNRINFO("ORDERBY",1)="[SYSDEV,DBTBL1F]%LIBS"
 .	S VBNRINFO("ORDERBY",2)="[SYSDEV,DBTBL1F]19"
 .	S VBNRINFO("ORDERBY",3)="[SYSDEV,DBTBL1F]TBLREF"
 .	S VBNRINFO("ORDERBY",4)="[SYSDEV,DBTBL1F]FID"
 ...
 N rwrs,vos1,vos2,sqlcur,exe,sqlcur,vd,vi,vsql,vsub S rwrs=$$vOpen0(.exe,.vsql,"DBTBL1F.%LIBS,DBTBL1F.TBLREF,DBTBL1F.FID,DBTBL1F.FKEYS,DBTBL1.DES,DBTBL1F.UPD,DBTBL1F.DEL,DBTBL1F.PKEYS","DBTBL1F,DBTBL1",VWHERE,"DBTBL1F.%LIBS,DBTBL1F.TBLREF,DBTBL1F.FID,DBTBL1F.FKEYS","","/DQMODE=1",1)
 ;  #ACCEPT Date=10/13/2008;Pgm=RussellDS;CR=35741;Group=READ
...
VGETDATA(V,VI) ; 
 ...
 S vc2=$piece(V,$char(9),2) ; DBTBL1F.TBLREF
 S vc3=$piece(V,$char(9),3) ; DBTBL1F.FID
 S vc4=$piece(V,$char(9),4) ; DBTBL1F.FKEYS
 S vc5=$piece(V,$char(9),5) ; DBTBL1.DES
 S vc6=$piece(V,$char(9),6) ; DBTBL1F.UPD
 S vc7=$piece(V,$char(9),7) ; DBTBL1F.DEL
...
VBREAK ; 
 ...
 I vb3!(vovc2'=vc2) S vs(4)=0 S vh(4)=0 S VD(3)=0 S vb4=1 S vb5=1
...
VHDG5 ; Group Header
 ...
 ;
 D VP1 Q:VFMQ!$get(verror)  S V=$E(zref,1,30) S VL="  "_V
 D VOM
 Q 
 ;
VDTL5 ; Detail
 ...
 ;
 S VL="                      "_$E(vc5,1,40)
 S VL=VL_$J("",(66-$L(VL)))_$E(vc3,1,35)
 S VL=VL_$J("",(104-$L(VL)))_"Update : "
 S VL=VL_$J("",(113-$L(VL)))_$J(vc6,1)
 S V=$$VP2(vc6) ; @TBL([DBTBL1F]UPD)
 S VL=VL_$J("",(117-$L(VL)))_$E(V,1,12)
 D VOM
 S VL="                                                                                                        "_"Delete : "
 S VL=VL_""_$J(vc7,1)
 S V=$$VP3(vc7) ; @TBL([DBTBL1F]DEL)
 S VL=VL_$J("",(117-$L(VL)))_$E(V,1,12)
 D VOM
 D VOM
 Q 
...
VHDG0 ; Page Header
 ...
 S VL="  "_"Foreign"
 D VOM
 S VL=" "_" File Name(Key)      Foreign Key Reference                       File Name                             Constraint"
 D VOM
 S VL="------------------------------------------------------------------------------------------------------------------------------------"
 D VOM
 ;
