 ...
 ; 
 ;DO NOT MODIFY  Local procedure runtime driver|DBSDRV|||||||1
 ;  #PACKAGE framework
 ...
 N %UserID S %UserID=$$USERNAM^%ZFUNC ; replace with Runtime.username once available
 ;
 N args
 N status
 ;
...
run(args) ; 
 ...
 ..		;
 ..		N dbsfuncucls,vop3 S dbsfuncucls=$$vRCgetRecord1Opt^RecordDBSFUNCUCLS(func,%UserClass,0,.vop3)
 ..		;
...
rpclpc(rpcReturn,version,function,cmdStr) ; Command string [*]
 ...
 ;
 N scatbl5a,vop1 S scatbl5a=$$vCa3(function,%UserClass,.vop1)
 ;
...
rpcrun(rpcReturn,version,runable,cmdStr) ; Command string [*]
 ...
 ;
 N scatbl5a,vop1 S scatbl5a=$$vCa3(runable,%UserClass,.vop1)
 ;
 ; Determine if authorized for this userclass
...
build(isRPC,args) ; 
 ...
 ;
 S $P(vobj(iofile,1),"|",1)="/bip/spool/DBSDRVDSP.tmp"
 ...
 .	D write^UCIO(iofile," // Last compiled:  "_%CurrentDate_" "_%CurrentTime_" - "_%UserName)
 .	D write^UCIO(iofile,"")
 ...
 S status=$$storePsl^PSLC("/bip/spool/DBSDRVDSP.tmp","DBSDRVDSP")
 ...
 ;
 ;  #ACCEPT Date=02/22/2009; Pgm=RussellDS; CR=35741; Group=ACCESS
 S status=$$DELETE^%OSSCRPT("/bip/spool/DBSDRVDSP.tmp")
 ;
...
cmdline(cmdStr,args) ; Decomposed command line arguments/options
 ...
 ;  #OPTION ResultClass ON
vSIG() ; 
 Q "^^^17183" ; Signature - LTD^TIME^USER^SIZE
 ; ----------------
