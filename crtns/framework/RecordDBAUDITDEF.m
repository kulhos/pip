 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordDBAUDITDEF ****
 ; 
 ; 02/24/2010 18:39 - pip
 ; 
 ;
 ; Record Class code for table DBAUDITDEF
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBAUDITDEF",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBAUDITDEF"
 S vobj(vOid)=$G(^DBAUDITDEF(v1,v2))
 I vobj(vOid)="",'($D(^DBAUDITDEF(v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBAUDITDEF",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S dbauditdef=$G(^DBAUDITDEF(v1,v2))
 I dbauditdef="",'($D(^DBAUDITDEF(v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBAUDITDEF",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q dbauditdef
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^DBAUDITDEF(V1,V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,dbauditdef,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DELETELOG,INSERTLOG,SELECTLOG,TABLENAME,UPDATELOG,USERCLASS",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ...
 S map(-4)="USERCLASS:T:"
 S map(-3)="TABLENAME:T:"
 S map(-1)="DELETELOG:N:3;INSERTLOG:N:1;SELECTLOG:N:4;UPDATELOG:N:2"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N dbauditdef S dbauditdef=$$vRCgetRecord0^RecordDBAUDITDEF(TABLENAME,USERCLASS,0)
 .	S vobj(dbauditdef,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBAUDITDEF(dbauditdef,$$initPar^UCUTILN(params)) K vobj(dbauditdef,-100) S vobj(dbauditdef,-2)=1 TC:vTp  
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^DBAUDITDEF(V1,V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^DBAUDITDEF(vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^DBAUDITDEF(vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBAUDITDEF","MSG",1767,"DBAUDITDEF."_column)
 I ER D throwError($get(RM))
 Q 
 ;
vRCTrig1(this,dbauditdef,vpar) ; Trigger BEFORE_INSERT - Before Insert - BI
 ;
  S ER=0
 ;
  N V1 S V1=vobj(dbauditdef,-4) I ('($D(^SCAU(0,V1))#2)!(vobj(dbauditdef,-4)="PUBLIC")) D  Q 
 .	;
 .	S ER=1
 .	; Invalid userclass ~p1
 .	S RM=$$^MSG(6755,vobj(dbauditdef,-4))
 .	Q 
 ;
 Q 
 ;
...
vRCvalidateDD(this,processMode) ; 
 ...
 S X=$P(vobj(this),$C(124),3) I '(X=""),'$$vCaEx1() D vRCvalidateDDerr("DELETELOG",$$^MSG(1485,X))
 S X=$P(vobj(this),$C(124),1) I '(X=""),'$$vCaEx2() D vRCvalidateDDerr("INSERTLOG",$$^MSG(1485,X))
 S X=$P(vobj(this),$C(124),4) I '(X=""),'$$vCaEx3() D vRCvalidateDDerr("SELECTLOG",$$^MSG(1485,X))
 S X=$P(vobj(this),$C(124),2) I '(X=""),'$$vCaEx4() D vRCvalidateDDerr("UPDATELOG",$$^MSG(1485,X))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"0*","DELETELOG"))&($P($E($G(vobj(this,-100,"0*","DELETELOG")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) S X=$P(vobj(this),$C(124),3) I '(X=""),'$$vCaEx5() D vRCvalidateDDerr("DELETELOG",$$^MSG(1485,X))
 I ($D(vobj(this,-100,"0*","INSERTLOG"))&($P($E($G(vobj(this,-100,"0*","INSERTLOG")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) S X=$P(vobj(this),$C(124),1) I '(X=""),'$$vCaEx6() D vRCvalidateDDerr("INSERTLOG",$$^MSG(1485,X))
 I ($D(vobj(this,-100,"0*","SELECTLOG"))&($P($E($G(vobj(this,-100,"0*","SELECTLOG")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) S X=$P(vobj(this),$C(124),4) I '(X=""),'$$vCaEx7() D vRCvalidateDDerr("SELECTLOG",$$^MSG(1485,X))
 I ($D(vobj(this,-100,"0*","UPDATELOG"))&($P($E($G(vobj(this,-100,"0*","UPDATELOG")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) S X=$P(vobj(this),$C(124),2) I '(X=""),'$$vCaEx8() D vRCvalidateDDerr("UPDATELOG",$$^MSG(1485,X))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBAUDITDEF","MSG",979,"DBAUDITDEF."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","USERCLASS")):$P($E(vobj(this,-100,"2*","USERCLASS"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^DBAUDITDEF(V1,V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBAUDITDEF(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("DBAUDITDEF",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrRep(object,p1,p2,p3,p4,qt) ; String.replace
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx1() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx2() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 N vRec,vop1 S vRec=$$vCa2(X,.vop1)
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx3() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 N vRec,vop1 S vRec=$$vCa2(X,.vop1)
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx4() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 N vRec,vop1 S vRec=$$vCa2(X,.vop1)
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx5() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 N vRec,vop1 S vRec=$$vCa2(X,.vop1)
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx6() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 N vRec,vop1 S vRec=$$vCa2(X,.vop1)
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx7() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 N vRec,vop1 S vRec=$$vCa2(X,.vop1)
 ...
 ; ----------------
 ;  #OPTION ResultClass 1
vCaEx8() ; {Cache}%CACHE("STBLDBAUDLOG").isDefined("STBLDBAUDLOG","OPTION=:X")
 N vret
 ;
 ;  #OPTIMIZE FUNCTIONS OFF
 N vRec,vop1 S vRec=$$vCa2(X,.vop1)
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vCa2(v1,v2out) ; voXN = ({Cache}%CACHE("STBLDBAUDLOG").getRecord(STBLDBAUDLOG,1)
 ;
 I '$D(%CACHE("STBLDBAUDLOG",v1)) D
 .  I $G(%CACHE("STBLDBAUDLOG"))>100 KILL %CACHE("STBLDBAUDLOG")
 .  S %CACHE("STBLDBAUDLOG")=$G(%CACHE("STBLDBAUDLOG"))+1
 .  S %CACHE("STBLDBAUDLOG",v1)=$$vRCgetRecord1Opt^RecordSTBLDBAUDLOG(v1,0,.v2out),%CACHE("STBLDBAUDLOG",v1,-2)=v2out
 ;
 ;
 E  S v2out=%CACHE("STBLDBAUDLOG",v1,-2)
 Q %CACHE("STBLDBAUDLOG",v1)
 ;
vReCp1(v1) ; RecordDBAUDITDEF.copy: DBAUDITDEF
 ;
 Q $$copy^UCGMR(this)
