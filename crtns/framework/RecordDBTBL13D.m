 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordDBTBL13D ****
 ; 
 ; 02/24/2010 18:39 - pip
 ; 
 ;
 ; Record Class code for table DBTBL13D
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBTBL13D",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
 S vobj(vOid,-5)=""
...
vRCgetRecord0(v1,v2,v3,vfromDbSet) ; 
 ...
 I '(v3>0)
 E  I vobj(vOid)="",'($D(^DBTBL(v1,13,v2,v3))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBTBL13D",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 S vobj(vOid,-5)=v3
 ;*** End of code by-passed by compiler ***
...
vRCgetRecord0Opt(v1,v2,v3,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S dbtbl13d=$G(^DBTBL(v1,13,v2,v3))
 I '(v3>0)
 E  I dbtbl13d="",'($D(^DBTBL(v1,13,v2,v3))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBTBL13D",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q dbtbl13d
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","LIBS"))&($P($E($G(vobj(this,-100,"1*","LIBS")),5,9999),$C(12))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","PID"))&($P($E($G(vobj(this,-100,"2*","PID")),5,9999),$C(12))'=vobj(this,-4)))!($D(vobj(this,-100,"3*","SEQ"))&($P($E($G(vobj(this,-100,"3*","SEQ")),5,9999),$C(12))'=vobj(this,-5))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,dbtbl13d,vkey1,vkey2,vkey3) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DATA,LIBS,PID,SEQ",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N dbtbl13d S dbtbl13d=$$vRCgetRecord0^RecordDBTBL13D(LIBS,PID,SEQ,0)
 .	S vobj(dbtbl13d,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBTBL13D(dbtbl13d,$$initPar^UCUTILN(params)) K vobj(dbtbl13d,-100) S vobj(dbtbl13d,-2)=1 TC:vTp  
 .	K vobj(+$G(dbtbl13d)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^DBTBL(vobj(this,-3),13,vobj(this,-4),vobj(this,-5))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^DBTBL(vobj(this,-3),13,vobj(this,-4),vobj(this,-5))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBTBL13D","MSG",1767,"DBTBL13D."_column)
...
vRCvalidateDD1(this) ; 
 ...
 I (vobj(this,-4)'=$ZCONVERT(vobj(this,-4),"U")) D vRCvalidateDDerr("PID",$$^MSG(1476))
 I ($D(vobj(this,-100,"2*","PID"))&($P($E($G(vobj(this,-100,"2*","PID")),5,9999),$C(12))'=vobj(this,-4))) I ($L(vobj(this,-4))>12) D vRCvalidateDDerr("PID",$$^MSG(1076,12))
...
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBTBL13D","MSG",979,"DBTBL13D."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","PID")):$P($E(vobj(this,-100,"2*","PID"),5,9999),$C(12)),1:vobj(this,-4))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBTBL13D(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("DBTBL13D",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vDbEx2() ; min(1): DISTINCT LIBS,PID,SEQ FROM DBTBL13D WHERE LIBS=:V1 and PID=:V2 and SEQ=:V3
 ...
 Q 1
 ;
vDbEx3() ; min(1): DISTINCT LIBS,PID,SEQ FROM DBTBL13D WHERE LIBS=:V1 and PID=:V2 and SEQ=:V3
 ;
 N vsql1
 S vsql1=$$BYTECHAR^SQLUTL(254)
 ;
