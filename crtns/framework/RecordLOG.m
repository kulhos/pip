 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordLOG ****
 ; 
 ; 02/24/2010 18:40 - pip
 ; 
 ;
 ; Record Class code for table LOG
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordLOG",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordLOG"
 S vobj(vOid)=$G(^LOG(v1,v2))
 I '(v1>0)
 E  I vobj(vOid)="",'($D(^LOG(v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,LOG",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 S log=$G(^LOG(v1,v2))
 I '(v1>0)
 E  I log="",'($D(^LOG(v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,LOG",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q log
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","TJD"))&($P($E($G(vobj(this,-100,"1*","TJD")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","SEQ"))&($P($E($G(vobj(this,-100,"2*","SEQ")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'$$vDbEx1() 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,log,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("%O,ACKEYS,FID,KEYVALUE,LIBS,SEQ,TIME,TJD,TLO,UID",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ...
 S map(-4)="SEQ:N:"
 S map(-3)="TJD:D:"
 S map(-1)="%O:N:4;ACKEYS:T:2;FID:T:1;KEYVALUE:T:3;LIBS:T:7;TIME:C:8;TLO:T:6;UID:T:5"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N log S log=$$vRCgetRecord0^RecordLOG(TJD,SEQ,0)
 .	S vobj(log,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordLOG(log,$$initPar^UCUTILN(params)) K vobj(log,-100) S vobj(log,-2)=1 TC:vTp  
 .	K vobj(+$G(log)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''$$vDbEx2()=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^LOG(vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^LOG(vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("LOG","MSG",1767,"LOG."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 S X=$P(vobj(this),$C(124),4) I '(X=""),(X'?1N) D vRCvalidateDDerr("%O",$$^MSG(742,"N"))
 I ($L($P(vobj(this),$C(124),2))>256) D vRCvalidateDDerr("ACKEYS",$$^MSG(1076,256))
 I ($L($P(vobj(this),$C(124),1))>256) D vRCvalidateDDerr("FID",$$^MSG(1076,256))
 I ($L($P(vobj(this),$C(124),3))>256) D vRCvalidateDDerr("KEYVALUE",$$^MSG(1076,256))
 I ($L($P(vobj(this),$C(124),7))>12) D vRCvalidateDDerr("LIBS",$$^MSG(1076,12))
 S X=$P(vobj(this),$C(124),8) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("TIME",$$^MSG(742,"C"))
 I ($L($P(vobj(this),$C(124),6))>40) D vRCvalidateDDerr("TLO",$$^MSG(1076,40))
 I ($L($P(vobj(this),$C(124),5))>12) D vRCvalidateDDerr("UID",$$^MSG(1076,12))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"0*","%O"))&($P($E($G(vobj(this,-100,"0*","%O")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))),'($P(vobj(this),$C(124),4)=""),'(+$P(vobj(this),$C(124),4)=$P(vobj(this),$C(124),4))  S $P(vobj(this),$C(124),4)=$$vRCtrimNumber($P(vobj(this),$C(124),4))
 I ($D(vobj(this,-100,"0*","%O"))&($P($E($G(vobj(this,-100,"0*","%O")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) S X=$P(vobj(this),$C(124),4) I '(X=""),(X'?1N) D vRCvalidateDDerr("%O",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","ACKEYS"))&($P($E($G(vobj(this,-100,"0*","ACKEYS")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>256) D vRCvalidateDDerr("ACKEYS",$$^MSG(1076,256))
 I ($D(vobj(this,-100,"0*","FID"))&($P($E($G(vobj(this,-100,"0*","FID")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>256) D vRCvalidateDDerr("FID",$$^MSG(1076,256))
 I ($D(vobj(this,-100,"0*","KEYVALUE"))&($P($E($G(vobj(this,-100,"0*","KEYVALUE")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I ($L($P(vobj(this),$C(124),3))>256) D vRCvalidateDDerr("KEYVALUE",$$^MSG(1076,256))
 I ($D(vobj(this,-100,"0*","LIBS"))&($P($E($G(vobj(this,-100,"0*","LIBS")),5,9999),$C(124))'=$P(vobj(this),$C(124),7))) I ($L($P(vobj(this),$C(124),7))>12) D vRCvalidateDDerr("LIBS",$$^MSG(1076,12))
 I ($D(vobj(this,-100,"0*","TIME"))&($P($E($G(vobj(this,-100,"0*","TIME")),5,9999),$C(124))'=$P(vobj(this),$C(124),8))) S X=$P(vobj(this),$C(124),8) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("TIME",$$^MSG(742,"C"))
 I ($D(vobj(this,-100,"0*","TLO"))&($P($E($G(vobj(this,-100,"0*","TLO")),5,9999),$C(124))'=$P(vobj(this),$C(124),6))) I ($L($P(vobj(this),$C(124),6))>40) D vRCvalidateDDerr("TLO",$$^MSG(1076,40))
 I ($D(vobj(this,-100,"0*","UID"))&($P($E($G(vobj(this,-100,"0*","UID")),5,9999),$C(124))'=$P(vobj(this),$C(124),5))) I ($L($P(vobj(this),$C(124),5))>12) D vRCvalidateDDerr("UID",$$^MSG(1076,12))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("LOG","MSG",979,"LOG."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","SEQ")):$P($E(vobj(this,-100,"2*","SEQ"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I $$vDbEx3() D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordLOG(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("LOG",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vDbEx1() ; min(1): DISTINCT TJD,SEQ FROM LOG WHERE TJD=:V1 and SEQ=:V2
 ;
 N vsql1
 ...
 Q 1
 ;
vDbEx2() ; min(1): DISTINCT TJD,SEQ FROM LOG WHERE TJD=:V1 and SEQ=:V2
 ;
 N vsql1
 S vsql1=$$BYTECHAR^SQLUTL(254)
 ;
 ...
 Q 1
 ;
vDbEx3() ; min(1): DISTINCT TJD,SEQ FROM LOG WHERE TJD=:V1 and SEQ=:V2
 ;
 N vsql1
 S vsql1=$$BYTECHAR^SQLUTL(254)
 ;
