...
extdata(dbmap) ; DBMAP Record
 N vret
  N von,vol S von="",vol=$D(vobj(dbmap,1,1)) S:'vol vobj(dbmap,1,1)="" F  Q:vol!'vobj(dbmap,-2)  S von=$O(^DBMAP("COLUMNS",vobj(dbmap,-3),vobj(dbmap,-4),vobj(dbmap,-5),von)) quit:von=""  S vobj(dbmap,1,1)=vobj(dbmap,1,1)_^DBMAP("COLUMNS",vobj(dbmap,-3),vobj(dbmap,-4),vobj(dbmap,-5),von)
 ;
...
FILE(dqtable,dbid) ; Database ID
 ...
 .	;
 .	N rs,vos1,vos2,vos3,vos4  N V2 S V2=parent S rs=$$vOpen2()
 ...
 .	E  D
 ..		;
 ..		N rs,vos5,vos6,vos7,vos8  N V2 S V2=dqtable S rs=$$vOpen5()
 ...
 .	;
 .	N rs,vos9,vos10,vos11,vos12  N V2 S V2=dqtable S rs=$$vOpen6()
 .	;
 .	F  Q:'$$vFetch6()  D
 ..		;
 ..  S dqcol=rs
...
ADDMISNG(dbid,table,MAP,SPLIT) ; Split table for missing 100 or null
 ...
 ;
 N rs,vos1,vos2,vos3,vos4,vos5,vOid  N V1 S V1=table S rs=$$vOpen9()
 ;
 F  Q:'$$vFetch9()  D
 .	;
 . S COLUMN=rs
...
vL3a1 S vobj(vOid,1)=$$BYTECHAR^SQLUTL(254)
 S vobj(vOid,2)=""
vL3a3 S vobj(vOid,2)=$O(^STBL("RESERVED",vobj(vOid,2)),1) I vobj(vOid,2)="" G vL3a0
 Q
 ;
vFetch3(vOid) ;
 ;
 ...
 ;
 S vobj(vOid)=$S(vobj(vOid,2)=vobj(vOid,1):"",1:vobj(vOid,2))
 ;
 Q 1
 ;
...
vL5a1 S vos6=$$BYTECHAR^SQLUTL(254)
 S vos7=$G(V2) I vos7="" G vL5a0
 S vos8=""
vL5a4 S vos8=$O(^DBTBL("SYSDEV",1,vos7,9,vos8),1) I vos8="" G vL5a0
 Q
 ;
vFetch5() ;
 ;
...
vOpen6() ; DI FROM DBTBL1D WHERE %LIBS='SYSDEV' AND FID=:V2
 ...
 ;
vL6a0 S vos9=0 Q
vL6a1 S vos10=$$BYTECHAR^SQLUTL(254)
 S vos11=$G(V2) I vos11="" G vL6a0
 S vos12=""
vL6a4 S vos12=$O(^DBTBL("SYSDEV",1,vos11,9,vos12),1) I vos12="" G vL6a0
 Q
 ;
vFetch6() ;
 ;
...
vL7a1 S vobj(vOid,1)=$$BYTECHAR^SQLUTL(254)
 S vobj(vOid,2)=$G(V1) I vobj(vOid,2)="" G vL7a0
 I '($D(^STBL("RESERVED",vobj(vOid,2)))#2) G vL7a0
 Q
 ;
vFetch7(vOid) ;
 ;
...
vL9a1 S vos2=$$BYTECHAR^SQLUTL(254)
 ...
 S vos4=""
vL9a4 S vos4=$O(^DBTBL(vos4),1) I vos4="" G vL9a10
 S vos5=""
vL9a6 S vos5=$O(^DBTBL(vos4,1,vos3,9,vos5),1) I vos5="" G vL9a4
 S vd=$S(vos5=vos2:"",1:vos5)
 S ^DBTMP($J,vOid,1,vos5,vos4)=vd
 G vL9a6
vL9a10 S vos2=""
vL9a11 S vos2=$O(^DBTMP($J,vOid,1,vos2),1) I vos2="" G vL9a0
 S vos3=""
vL9a13 S vos3=$O(^DBTMP($J,vOid,1,vos2,vos3),1) I vos3="" G vL9a11
 Q
 ;
vFetch9() ;
 ;
