 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordDBTBL4D ****
 ; 
 ; 02/24/2010 18:40 - pip
 ; 
 ;
 ; Record Class code for table DBTBL4D
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBTBL4D",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
 S vobj(vOid,-5)=""
...
vRCgetRecord0(v1,v2,v3,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBTBL4D"
 S vobj(vOid)=$G(^DBTBL(v1,4,v2,v3))
 I '(v3>0)
 E  I vobj(vOid)="",'($D(^DBTBL(v1,4,v2,v3))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBTBL4D",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 S vobj(vOid,-5)=v3
 ;*** End of code by-passed by compiler ***
...
vRCgetRecord0Opt(v1,v2,v3,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S dbtbl4d=$G(^DBTBL(v1,4,v2,v3))
 I '(v3>0)
 E  I dbtbl4d="",'($D(^DBTBL(v1,4,v2,v3))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBTBL4D",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q dbtbl4d
 ;
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,dbtbl4d,vkey1,vkey2,vkey3) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("LIBS,LINE,QID,SEQ",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N dbtbl4d S dbtbl4d=$$vRCgetRecord0^RecordDBTBL4D(LIBS,QID,SEQ,0)
 .	S vobj(dbtbl4d,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBTBL4D(dbtbl4d,$$initPar^UCUTILN(params)) K vobj(dbtbl4d,-100) S vobj(dbtbl4d,-2)=1 TC:vTp  
 .	K vobj(+$G(dbtbl4d)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^DBTBL(vobj(this,-3),4,vobj(this,-4),vobj(this,-5))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^DBTBL(vobj(this,-3),4,vobj(this,-4),vobj(this,-5))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBTBL4D","MSG",1767,"DBTBL4D."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 I ($L($P(vobj(this),$C(124),1))>75) D vRCvalidateDDerr("LINE",$$^MSG(1076,75))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"0*","LINE"))&($P($E($G(vobj(this,-100,"0*","LINE")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>75) D vRCvalidateDDerr("LINE",$$^MSG(1076,75))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBTBL4D","MSG",979,"DBTBL4D."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBTBL4D(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("DBTBL4D",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vDbEx1() ; min(1): DISTINCT LIBS,QID,SEQ FROM DBTBL4D WHERE LIBS=:V1 and QID=:V2 and SEQ=:V3
 ;
 N vsql1
 ...
 ;
 I '(V3>0) Q 0
 I '($D(^DBTBL(V1,4,V2,V3))#2) Q 0
 Q 1
 ;
vDbEx2() ; min(1): DISTINCT LIBS,QID,SEQ FROM DBTBL4D WHERE LIBS=:V1 and QID=:V2 and SEQ=:V3
 ;
 N vsql1
 S vsql1=$$BYTECHAR^SQLUTL(254)
 ;
 ...
 Q 1
 ;
vDbEx3() ; min(1): DISTINCT LIBS,QID,SEQ FROM DBTBL4D WHERE LIBS=:V1 and QID=:V2 and SEQ=:V3
 ;
 N vsql1
 S vsql1=$$BYTECHAR^SQLUTL(254)
 ;
 ...
 Q 1
 ;
vReCp1(v1) ; RecordDBTBL4D.copy: DBTBL4D
 ;
 Q $$copy^UCGMR(this)
