 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordSYSMAPLABELS ****
 ; 
 ; 02/24/2010 18:41 - pip
 ; 
 ;
 ; Record Class code for table SYSMAPLABELS
 ;
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 I vobj(vOid)="",'($D(^SYSMAP("LABELS",v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,SYSMAPLABELS",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S sysmaplabels=$G(^SYSMAP("LABELS",v1,v2))
 I sysmaplabels="",'($D(^SYSMAP("LABELS",v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,SYSMAPLABELS",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q sysmaplabels
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","TARGET"))&($P($E($G(vobj(this,-100,"1*","TARGET")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","LABEL"))&($P($E($G(vobj(this,-100,"2*","LABEL")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^SYSMAP("LABELS",V1,V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,sysmaplabels,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("LABEL,TARGET",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q $$vStrRep("PARAMS",",",$char(9),0,0,"")
 ;
columnListCMP() ; 
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N sysmaplabels S sysmaplabels=$$vRCgetRecord0^RecordSYSMAPLABELS(TARGET,LABEL,0)
 .	S vobj(sysmaplabels,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSYSMAPLABELS(sysmaplabels,$$initPar^UCUTILN(params)) K vobj(sysmaplabels,-100) S vobj(sysmaplabels,-2)=1 TC:vTp  
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^SYSMAP("LABELS",V1,V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) K:$D(vobj(this,1,1)) ^SYSMAP("LABELS",vobj(this,-3),vobj(this,-4)) S ^SYSMAP("LABELS",vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	;    #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; GROUP=BYPASS
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this,1,1)) N vS1,vS2 S vS1=0 F vS2=1:450:$ZL(vobj(this,1,1)) S vS1=vS1+1,^SYSMAP("LABELS",vobj(this,-3),vobj(this,-4),vS1)=$ZE(vobj(this,1,1),vS2,vS2+449)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 kill ^SYSMAP("LABELS",vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("SYSMAPLABELS","MSG",1767,"SYSMAPLABELS."_column)
...
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("SYSMAPLABELS","MSG",979,"SYSMAPLABELS."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","LABEL")):$P($E(vobj(this,-100,"2*","LABEL"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^SYSMAP("LABELS",V1,V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSYSMAPLABELS(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("SYSMAPLABELS",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vReCp1(v1) ; RecordSYSMAPLABELS.copy: SYSMAPLABELS
 ...
 N vNod,vOid
 I $G(vobj(v1,-2)) D
 . N von,vol S von="",vol=$D(vobj(v1,1,1)) S:'vol vobj(v1,1,1)="" F  Q:vol  S von=$O(^SYSMAP("LABELS",vobj(v1,-3),vobj(v1,-4),von)) quit:von=""  S vobj(v1,1,1)=vobj(v1,1,1)_^SYSMAP("LABELS",vobj(v1,-3),vobj(v1,-4),von)
