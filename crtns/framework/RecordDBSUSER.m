 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordDBSUSER ****
 ; 
 ; 02/24/2010 18:39 - pip
 ; 
 ;
 ; Record Class code for table DBSUSER
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBSUSER",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBSUSER"
 S vobj(vOid)=$G(^DBSUSER(v1))
 I vobj(vOid)="",'($D(^DBSUSER(v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBSUSER",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S dbsuser=$G(^DBSUSER(v1))
 I dbsuser="",'($D(^DBSUSER(v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBSUSER",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q dbsuser
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^DBSUSER(V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,dbsuser,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("NOACCESS,UCLS,UID,USERNAME",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="UID:T:"
 S map(-1)="NOACCESS:L:3;UCLS:T:2;USERNAME:T:1"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N dbsuser S dbsuser=$$vRCgetRecord0^RecordDBSUSER(UID,0)
 .	S vobj(dbsuser,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBSUSER(dbsuser,$$initPar^UCUTILN(params)) K vobj(dbsuser,-100) S vobj(dbsuser,-2)=1 TC:vTp  
 .	K vobj(+$G(dbsuser)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^DBSUSER(V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^DBSUSER(vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
...
vRCchkReqForInsert(this) ; 
 I ($P(vobj(this),$C(124),3)="") D vRCrequiredErr("NOACCESS")
 I ($P(vobj(this),$C(124),2)="") D vRCrequiredErr("UCLS")
 I (vobj(this,-3)="") D vRCrequiredErr("UID")
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("USERNAME")
 Q 
 ;
vRCchkReqForUpdate(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("UID")
 I ($D(vobj(this,-100,"0*","NOACCESS"))&($P($E($G(vobj(this,-100,"0*","NOACCESS")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))),($P(vobj(this),$C(124),3)="") D vRCrequiredErr("NOACCESS")
 I ($D(vobj(this,-100,"0*","UCLS"))&($P($E($G(vobj(this,-100,"0*","UCLS")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))),($P(vobj(this),$C(124),2)="") D vRCrequiredErr("UCLS")
 I ($D(vobj(this,-100,"0*","USERNAME"))&($P($E($G(vobj(this,-100,"0*","USERNAME")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))),($P(vobj(this),$C(124),1)="") D vRCrequiredErr("USERNAME")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBSUSER","MSG",1767,"DBSUSER."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I ($L(vobj(this,-3))>25) D vRCvalidateDDerr("UID",$$^MSG(1076,25))
 I '(($P(vobj(this),$C(124),3)=1)!($P(vobj(this),$C(124),3)=0)) D vRCvalidateDDerr("NOACCESS",$$^MSG(742,"L"))
 S X=$P(vobj(this),$C(124),2) I '(X=""),'($D(^DBSUCLS(X))#2) D vRCvalidateDDerr("UCLS",$$^MSG(1485,X))
 I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("USERNAME",$$^MSG(1076,40))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","UID"))&($P($E($G(vobj(this,-100,"1*","UID")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>25) D vRCvalidateDDerr("UID",$$^MSG(1076,25))
 I ($D(vobj(this,-100,"0*","NOACCESS"))&($P($E($G(vobj(this,-100,"0*","NOACCESS")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I '(($P(vobj(this),$C(124),3)=1)!($P(vobj(this),$C(124),3)=0)) D vRCvalidateDDerr("NOACCESS",$$^MSG(742,"L"))
 I ($D(vobj(this,-100,"0*","UCLS"))&($P($E($G(vobj(this,-100,"0*","UCLS")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) S X=$P(vobj(this),$C(124),2) I '(X=""),'($D(^DBSUCLS(X))#2) D vRCvalidateDDerr("UCLS",$$^MSG(1485,X))
 I ($D(vobj(this,-100,"0*","USERNAME"))&($P($E($G(vobj(this,-100,"0*","USERNAME")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("USERNAME",$$^MSG(1076,40))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBSUSER","MSG",979,"DBSUSER."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","UID")):$P($E(vobj(this,-100,"1*","UID"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^DBSUSER(V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBSUSER(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("DBSUSER",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordDBSUSER.copy: DBSUSER
 ;
 Q $$copy^UCGMR(this)
