 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordDBSPID ****
 ; 
 ; 02/24/2010 18:39 - pip
 ; 
 ;
 ; Record Class code for table DBSPID
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBSPID",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
 S vobj(vOid,-5)=""
...
vRCgetRecord0(v1,v2,v3,vfromDbSet) ; 
 ...
 I vobj(vOid)="",'($D(^DBSPID(v1,v2,v3))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBSPID",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 S vobj(vOid,-5)=v3
 ;*** End of code by-passed by compiler ***
...
vRCgetRecord0Opt(v1,v2,v3,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S dbspid=$G(^DBSPID(v1,v2,v3))
 I dbspid="",'($D(^DBSPID(v1,v2,v3))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBSPID",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q dbspid
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","RTBL"))&($P($E($G(vobj(this,-100,"1*","RTBL")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","METHOD"))&($P($E($G(vobj(this,-100,"2*","METHOD")),5,9999),$C(124))'=vobj(this,-4)))!($D(vobj(this,-100,"3*","SEQ"))&($P($E($G(vobj(this,-100,"3*","SEQ")),5,9999),$C(124))'=vobj(this,-5))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2,V3 S V1=vobj(this,-3),V2=vobj(this,-4),V3=vobj(this,-5) Q:'($D(^DBSPID(V1,V2,V3))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,dbspid,vkey1,vkey2,vkey3) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("LASTDATE,LASTTIME,METHOD,RTBL,SEQ,SPNAME",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q $$vStrRep("PROCDATA",",",$char(9),0,0,"")
 ;
columnListCMP() ; 
...
getColumnMap(map) ; 
 ...
 S map(-4)="METHOD:T:"
 S map(-3)="RTBL:T:"
 S map(-1)="LASTDATE:D:2;LASTTIME:C:3;SPNAME:T:1"
 S map("1,1")="PROCDATA:M:"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N dbspid S dbspid=$$vRCgetRecord0^RecordDBSPID(RTBL,METHOD,SEQ,0)
 .	S vobj(dbspid,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBSPID(dbspid,$$initPar^UCUTILN(params)) K vobj(dbspid,-100) S vobj(dbspid,-2)=1 TC:vTp  
 .	K vobj(+$G(dbspid)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2,V3 S V1=vobj(this,-3),V2=vobj(this,-4),V3=vobj(this,-5) I '(''($D(^DBSPID(V1,V2,V3))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) K:$D(vobj(this,1,1)) ^DBSPID(vobj(this,-3),vobj(this,-4),vobj(this,-5)) S ^DBSPID(vobj(this,-3),vobj(this,-4),vobj(this,-5))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	;    #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; GROUP=BYPASS
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this,1,1)) N vS1,vS2 S vS1=0 F vS2=1:450:$ZL(vobj(this,1,1)) S vS1=vS1+1,^DBSPID(vobj(this,-3),vobj(this,-4),vobj(this,-5),vS1)=$ZE(vobj(this,1,1),vS2,vS2+449)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 kill ^DBSPID(vobj(this,-3),vobj(this,-4),vobj(this,-5))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I (vobj(this,-4)="") D vRCrequiredErr("METHOD")
 I (vobj(this,-3)="") D vRCrequiredErr("RTBL")
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBSPID","MSG",1767,"DBSPID."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I ($L(vobj(this,-3))>24) D vRCvalidateDDerr("RTBL",$$^MSG(1076,24))
 ...
 S X=vobj(this,-5) I '(X=""),(X'?1.4N),(X'?1"-"1.3N) D vRCvalidateDDerr("SEQ",$$^MSG(742,"N"))
 S X=$P(vobj(this),$C(124),2) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LASTDATE",$$^MSG(742,"D"))
 S X=$P(vobj(this),$C(124),3) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LASTTIME",$$^MSG(742,"C"))
 I ($L($P(vobj(this),$C(124),1))>30) D vRCvalidateDDerr("SPNAME",$$^MSG(1076,30))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","RTBL"))&($P($E($G(vobj(this,-100,"1*","RTBL")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>24) D vRCvalidateDDerr("RTBL",$$^MSG(1076,24))
 ...
 I ($D(vobj(this,-100,"3*","SEQ"))&($P($E($G(vobj(this,-100,"3*","SEQ")),5,9999),$C(124))'=vobj(this,-5))) S X=vobj(this,-5) I '(X=""),(X'?1.4N),(X'?1"-"1.3N) D vRCvalidateDDerr("SEQ",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","LASTDATE"))&($P($E($G(vobj(this,-100,"0*","LASTDATE")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) S X=$P(vobj(this),$C(124),2) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LASTDATE",$$^MSG(742,"D"))
 I ($D(vobj(this,-100,"0*","LASTTIME"))&($P($E($G(vobj(this,-100,"0*","LASTTIME")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) S X=$P(vobj(this),$C(124),3) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LASTTIME",$$^MSG(742,"C"))
 I ($D(vobj(this,-100,"0*","SPNAME"))&($P($E($G(vobj(this,-100,"0*","SPNAME")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>30) D vRCvalidateDDerr("SPNAME",$$^MSG(1076,30))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBSPID","MSG",979,"DBSPID."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","RTBL")):$P($E(vobj(this,-100,"1*","RTBL"),5,9999),$C(124)),1:vobj(this,-3))
 ...
 N oldKey3 S oldKey3=$S($D(vobj(this,-100,"3*","SEQ")):$P($E(vobj(this,-100,"3*","SEQ"),5,9999),$C(124)),1:vobj(this,-5))
  N V1,V2,V3 S V1=vobj(this,-3),V2=vobj(this,-4),V3=vobj(this,-5) I ($D(^DBSPID(V1,V2,V3))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBSPID(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("DBSPID",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordDBSPID.copy: DBSPID
 ;
 N vNod,vOid
 I $G(vobj(v1,-2)) D
 . N von,vol S von="",vol=$D(vobj(v1,1,1)) S:'vol vobj(v1,1,1)="" F  Q:vol  S von=$O(^DBSPID(vobj(v1,-3),vobj(v1,-4),vobj(v1,-5),von)) quit:von=""  S vobj(v1,1,1)=vobj(v1,1,1)_^DBSPID(vobj(v1,-3),vobj(v1,-4),vobj(v1,-5),von)
