...
SQLBUF(expr,par,sqlsta,sqldta,sqlcnt,sqlind,tok) ; 
 ...
 ..		Q 
 .	I '(auth="") S expr="/SPV_AUTH="_auth_"/SPV_AUTH_END"_expr
 .	;
 ...
 .  S $P(vobj(dbbufcom),$C(124),1)=z
 .  S vobj(dbbufcom,1,1)=expr
 .	;
...
COMMIT(name,tok,par,sqldta) ; Process a buffer for commit
 ...
 N expr N sql N vcurval N vfkey N vsupv N zpar
 N rdb
 ;
 ; RDB flag
 S rdb=$$rdb^UCDBRT()
 ...
 .	;
 .	S sql(vseq)=vop4
 .	S zpar(vseq)=$P(dbbufcom,$C(124),1)
 .	S zpar(vseq)=zpar(vseq)_"/NOFKCHK=1"
 .	I $E(sql(vseq),1,10)="/SPV_AUTH=" D
 ..		S auth=$piece($piece(sql(vseq),"/SPV_AUTH_END",1),"/SPV_AUTH=",2)
 ..		S sql(vseq)=$piece(sql(vseq),"/SPV_AUTH_END",2,999)
 ..		F i=1:1:$L(auth,"##") D
 ...
 I $D(vrflg),$D(vsupv) D SPVOVR^PBSMSQL(.vrflg,.vsupv) I $D(vrflg) D ERROR Q 
 ;
