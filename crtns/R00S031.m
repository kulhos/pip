 ; 
 ; **** Routine compiled from DATA-QWIK Report DBAUDITLOG ****
 ; 
...
R00S031 ; DBAUDITLOG - Audit Log Report
 ...
 ;
 S RID="DBAUDITLOG"
 S RN="Audit Log Report"
 I $get(IO)="" S IO=$I
 ;
 D INIT^%ZM()
 ;
 S %TAB("IO")=$$IO^SCATAB
 S %TAB("VIN1")="|255||[DBAUDITLOG]CDATE|[DBAUDITLOG]CDATE:DISTINCT:NOVAL||D EXT^DBSQRY||T|Calendar Date|||||"
 S %TAB("VIN2")="|255||[DBAUDITLOG]USERID|||D EXT^DBSQRY||T|User ID|||||"
 S %TAB("VIN3")="|255||[DBAUDITLOG]TLO|||D EXT^DBSQRY||T|User Location|||||"
 S %TAB("VIN4")="|255||[DBAUDITLOG]OPERATION|||D EXT^DBSQRY||T|SQL Operation|||||"
 S %TAB("VIN5")="|255||[DBAUDITLOG]OPON|||D EXT^DBSQRY||T|Table or Procedure Operated On|||||"
 ;
...
V0 ; External report entry point
 ...
 N VWHERE
 N %TIM N CONAM N RID N RN N VL N VLOF N VRF N VSTATS N vCOL N vHDG N vc1 N vc2 N vc3 N vc4 N vc5 N vc6 N vc7 N vc8 N vc9 N vovc1 N vovc2 N vovc3 N vovc4 N vovc5 N vrundate N vsysdate
 ;
 N cuvar S cuvar=$$vRCgetRecord0Opt^RecordCUVAR(0,"")
  S cuvar=$G(^CUVAR("BANNER"))
 ;
 S CONAM="PIP Version 0.2"
 S ER=0 S RID="DBAUDITLOG" S RN="Audit Log Report"
 S VL=""
 ;
 ...
 ..		N CHRSET S CHRSET=$$^UCIOENCD("Report","DBAUDITLOG","V0","*")
 ...
 .	I VIN1'="ALL" S DQQRY(1)="[DBAUDITLOG]CDATE "_VIN1 S SEQ=SEQ+1
 .	I $get(VIN2)="" S VIN2="ALL"
 .	I VIN2'="ALL" S DQQRY(SEQ)="[DBAUDITLOG]USERID "_VIN2 S SEQ=SEQ+1
 .	I $get(VIN3)="" S VIN3="ALL"
 .	I VIN3'="ALL" S DQQRY(SEQ)="[DBAUDITLOG]TLO "_VIN3 S SEQ=SEQ+1
 .	I $get(VIN4)="" S VIN4="ALL"
 .	I VIN4'="ALL" S DQQRY(SEQ)="[DBAUDITLOG]OPERATION "_VIN4 S SEQ=SEQ+1
 .	I $get(VIN5)="" S VIN5="ALL"
 .	I VIN5'="ALL" S DQQRY(SEQ)="[DBAUDITLOG]OPON "_VIN5 S SEQ=SEQ+1
 .	S FROM=$$DQJOIN^SQLCONV("DBAUDITLOG") Q:ER 
 ...
 .	N VBNRINFO
 .	;
 .	S VBNRINFO("PROMPTS",1)="WC2|"_"Calendar Date"_"|VIN1|"_$get(VIN1)
 .	S VBNRINFO("PROMPTS",2)="WC2|"_"User ID"_"|VIN2|"_$get(VIN2)
 .	S VBNRINFO("PROMPTS",3)="WC2|"_"User Location"_"|VIN3|"_$get(VIN3)
 .	S VBNRINFO("PROMPTS",4)="WC2|"_"SQL Operation"_"|VIN4|"_$get(VIN4)
 .	S VBNRINFO("PROMPTS",5)="WC2|"_"Table or Procedure Operated On"_"|VIN5|"_$get(VIN5)
 .	;
 .	D
 ..		N SEQ
 ..		N VALUE N VAR N X
 ...
 ..		Q 
 .	;
 .	S VBNRINFO("DESC")="Audit Log Report"
 .	S VBNRINFO("PGM")="R00S031"
 .	S VBNRINFO("RID")="DBAUDITLOG"
 .	S VBNRINFO("TABLES")="DBAUDITLOG"
 .	;
 .	S VBNRINFO("ORDERBY",1)="[SYSDEV,DBAUDITLOG]OPON"
 .	S VBNRINFO("ORDERBY",2)="[SYSDEV,DBAUDITLOG]USERID"
 .	S VBNRINFO("ORDERBY",3)="[SYSDEV,DBAUDITLOG]CDATE"
 .	S VBNRINFO("ORDERBY",4)="[SYSDEV,DBAUDITLOG]PID"
 .	S VBNRINFO("ORDERBY",5)="[SYSDEV,DBAUDITLOG]SEQ"
 .	;
 ...
 N rwrs,vos1,vos2,sqlcur,exe,sqlcur,vd,vi,vsql,vsub S rwrs=$$vOpen0(.exe,.vsql,"DBAUDITLOG.OPON,DBAUDITLOG.USERID,DBAUDITLOG.CDATE,DBAUDITLOG.PID,DBAUDITLOG.SEQ,DBAUDITLOG.CTIME,DBAUDITLOG.TLO,DBAUDITLOG.STATEMENT,DBAUDITLOG.USING","DBAUDITLOG",VWHERE,"DBAUDITLOG.OPON,DBAUDITLOG.USERID,DBAUDITLOG.CDATE,DBAUDITLOG.PID,DBAUDITLOG.SEQ","","/DQMODE=1",1)
 ;  #ACCEPT Date=10/13/2008;Pgm=RussellDS;CR=35741;Group=READ
...
VSAVLAST ; Save last access keys values
 ...
 ;
VGETDATA(V,VI) ; 
 S vc1=$piece(V,$char(9),1) ; DBAUDITLOG.OPON
 S vc2=$piece(V,$char(9),2) ; DBAUDITLOG.USERID
 S vc3=$piece(V,$char(9),3) ; DBAUDITLOG.CDATE
 S vc4=$piece(V,$char(9),4) ; DBAUDITLOG.PID
 S vc5=$piece(V,$char(9),5) ; DBAUDITLOG.SEQ
 S vc6=$piece(V,$char(9),6) ; DBAUDITLOG.CTIME
 S vc7=$piece(V,$char(9),7) ; DBAUDITLOG.TLO
 S vc8=$piece(V,$char(9),8) ; DBAUDITLOG.STATEMENT
 S vc9=$piece(V,$char(9),9) ; DBAUDITLOG.USING
 Q 
 ;
...
VPRINT ; Print section
 N vskp
 ;
 I $get(VRWOPT("NODTL")) S vskp(5)=1 ; Skip detail
 D VBREAK
 D VSUM Q:VFMQ 
 ;
...
VDTL5 ; Detail
 ...
 ;
 S VL=$E(vc1,1,30)
 S VL=VL_$J("",(32-$L(VL)))_$E(vc2,1,20)
 S VL=VL_$J("",(54-$L(VL)))_$J($S(vc3'="":$ZD(vc3,"MM/DD/YEAR"),1:""),10)
 S VL=VL_$J("",(66-$L(VL)))_$J($$TIM^%ZM(vc6),10)
 S VL=VL_$J("",(76-$L(VL)))_$E(vc7,1,40)
 D VOM
 S VL="      "_$$MEMO(vc8,2007,100)
 D VOM
 S VL="        "_$$MEMO(vc9,3009,100)
...
VHDG0 ; Page Header
 ...
 D VOM
 S VL=$E(RN,1,30)
 S VL=VL_$J("",(31-$L(VL)))_"("
 S VL=VL_$J("",(32-$L(VL)))_$E(RID,1,12)
 S VL=VL_$J("",(44-$L(VL)))_")                                                         System:"
 S VL=VL_$J("",(110-$L(VL)))_$E(vsysdate,1,10)
 S VL=VL_$J("",(121-$L(VL)))_"Page: "
 S VL=VL_$J("",(127-$L(VL)))_$J(VPN,3)
 D VOM
 D VOM
 S VL="Table or Procedure              User ID               Date        Time      User Location"
 D VOM
 S VL="=================================================================================================================================="
 D VOM
 ;
