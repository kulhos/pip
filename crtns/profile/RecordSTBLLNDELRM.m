 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordSTBLLNDELRM ****
 ; 
 ; 02/24/2010 18:41 - pip
 ; 
 ;
 ; Record Class code for table STBLLNDELRM
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordSTBLLNDELRM",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordSTBLLNDELRM"
 S vobj(vOid)=$G(^STBL("LNDELRM",v1))
 I vobj(vOid)="",'($D(^STBL("LNDELRM",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,STBLLNDELRM",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S stbllndelrm=$G(^STBL("LNDELRM",v1))
 I stbllndelrm="",'($D(^STBL("LNDELRM",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,STBLLNDELRM",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q stbllndelrm
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^STBL("LNDELRM",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,stbllndelrm,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DSC,KEY,ROUTINE",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="KEY:N:"
 S map(-1)="DSC:T:1;ROUTINE:T:2"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N stbllndelrm S stbllndelrm=$$vRCgetRecord0^RecordSTBLLNDELRM(KEY,0)
 .	S vobj(stbllndelrm,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSTBLLNDELRM(stbllndelrm,$$initPar^UCUTILN(params)) K vobj(stbllndelrm,-100) S vobj(stbllndelrm,-2)=1 TC:vTp  
 .	K vobj(+$G(stbllndelrm)) Q 
 Q 
 ;
vhasLiterals() ; 
 Q 0
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^STBL("LNDELRM",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^STBL("LNDELRM",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^STBL("LNDELRM",vobj(this,-3))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("DSC")
 I (vobj(this,-3)="") D vRCrequiredErr("KEY")
 I ($P(vobj(this),$C(124),2)="") D vRCrequiredErr("ROUTINE")
 Q 
 ;
vRCchkReqForUpdate(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("KEY")
 I ($D(vobj(this,-100,"0*","DSC"))&($P($E($G(vobj(this,-100,"0*","DSC")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))),($P(vobj(this),$C(124),1)="") D vRCrequiredErr("DSC")
 I ($D(vobj(this,-100,"0*","ROUTINE"))&($P($E($G(vobj(this,-100,"0*","ROUTINE")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))),($P(vobj(this),$C(124),2)="") D vRCrequiredErr("ROUTINE")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("STBLLNDELRM","MSG",1767,"STBLLNDELRM."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 S X=vobj(this,-3) I '(X=""),(X'?1.2N),(X'?1"-"1.1N) D vRCvalidateDDerr("KEY",$$^MSG(742,"N"))
 I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DSC",$$^MSG(1076,40))
 I ($L($P(vobj(this),$C(124),2))>9) D vRCvalidateDDerr("ROUTINE",$$^MSG(1076,9))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"1*","KEY"))&($P($E($G(vobj(this,-100,"1*","KEY")),5,9999),$C(124))'=vobj(this,-3))) S X=vobj(this,-3) I '(X=""),(X'?1.2N),(X'?1"-"1.1N) D vRCvalidateDDerr("KEY",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","DSC"))&($P($E($G(vobj(this,-100,"0*","DSC")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DSC",$$^MSG(1076,40))
 I ($D(vobj(this,-100,"0*","ROUTINE"))&($P($E($G(vobj(this,-100,"0*","ROUTINE")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>9) D vRCvalidateDDerr("ROUTINE",$$^MSG(1076,9))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("STBLLNDELRM","MSG",979,"STBLLNDELRM."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","KEY")):$P($E(vobj(this,-100,"1*","KEY"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^STBL("LNDELRM",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSTBLLNDELRM(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("STBLLNDELRM",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordSTBLLNDELRM.copy: STBLLNDELRM
 ;
 Q $$copy^UCGMR(this)
