 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordDBCTLRFMTC ****
 ; 
 ; 02/24/2010 18:39 - pip
 ; 
 ;
 ; Record Class code for table DBCTLRFMTC
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBCTLRFMTC",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBCTLRFMTC"
 S vobj(vOid)=$G(^DBCTL("SYS","*RFMT",v1,v2))
 I vobj(vOid)="",'($D(^DBCTL("SYS","*RFMT",v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBCTLRFMTC",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S dbctlrfmtc=$G(^DBCTL("SYS","*RFMT",v1,v2))
 I dbctlrfmtc="",'($D(^DBCTL("SYS","*RFMT",v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBCTLRFMTC",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q dbctlrfmtc
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","CNTRY"))&($P($E($G(vobj(this,-100,"1*","CNTRY")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","TYPE"))&($P($E($G(vobj(this,-100,"2*","TYPE")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^DBCTL("SYS","*RFMT",V1,V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,dbctlrfmtc,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("CNTRY,DES,DFTJUS,EXDSP,NULLES,TYPE",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ...
 S map(-4)="TYPE:T:"
 S map(-3)="CNTRY:T:"
 S map(-1)="DES:T:1;DFTJUS:U:4;EXDSP:T:2;NULLES:T:3"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N dbctlrfmtc S dbctlrfmtc=$$vRCgetRecord0^RecordDBCTLRFMTC(CNTRY,TYPE,0)
 .	S vobj(dbctlrfmtc,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBCTLRFMTC(dbctlrfmtc,$$initPar^UCUTILN(params)) K vobj(dbctlrfmtc,-100) S vobj(dbctlrfmtc,-2)=1 TC:vTp  
 .	K vobj(+$G(dbctlrfmtc)) Q 
 Q 
 ;
vhasLiterals() ; 
 Q 0
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^DBCTL("SYS","*RFMT",V1,V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^DBCTL("SYS","*RFMT",vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^DBCTL("SYS","*RFMT",vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("CNTRY")
 I (vobj(this,-4)="") D vRCrequiredErr("TYPE")
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBCTLRFMTC","MSG",1767,"DBCTLRFMTC."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 I ($L(vobj(this,-4))>12) D vRCvalidateDDerr("TYPE",$$^MSG(1076,12))
 I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DES",$$^MSG(1076,40))
 I ($P(vobj(this),$C(124),4)'=$ZCONVERT($P(vobj(this),$C(124),4),"U")) D vRCvalidateDDerr("DFTJUS",$$^MSG(1476))
 I ($L($P(vobj(this),$C(124),4))>1) D vRCvalidateDDerr("DFTJUS",$$^MSG(1076,1))
 I ($L($P(vobj(this),$C(124),2))>78) D vRCvalidateDDerr("EXDSP",$$^MSG(1076,78))
 I ($L($P(vobj(this),$C(124),3))>20) D vRCvalidateDDerr("NULLES",$$^MSG(1076,20))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"2*","TYPE"))&($P($E($G(vobj(this,-100,"2*","TYPE")),5,9999),$C(124))'=vobj(this,-4))) I ($L(vobj(this,-4))>12) D vRCvalidateDDerr("TYPE",$$^MSG(1076,12))
 ...
 I ($P(vobj(this),$C(124),4)'=$ZCONVERT($P(vobj(this),$C(124),4),"U")) D vRCvalidateDDerr("DFTJUS",$$^MSG(1476))
 I ($D(vobj(this,-100,"0*","DFTJUS"))&($P($E($G(vobj(this,-100,"0*","DFTJUS")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) I ($L($P(vobj(this),$C(124),4))>1) D vRCvalidateDDerr("DFTJUS",$$^MSG(1076,1))
 I ($D(vobj(this,-100,"0*","EXDSP"))&($P($E($G(vobj(this,-100,"0*","EXDSP")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>78) D vRCvalidateDDerr("EXDSP",$$^MSG(1076,78))
 I ($D(vobj(this,-100,"0*","NULLES"))&($P($E($G(vobj(this,-100,"0*","NULLES")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I ($L($P(vobj(this),$C(124),3))>20) D vRCvalidateDDerr("NULLES",$$^MSG(1076,20))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBCTLRFMTC","MSG",979,"DBCTLRFMTC."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","TYPE")):$P($E(vobj(this,-100,"2*","TYPE"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^DBCTL("SYS","*RFMT",V1,V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBCTLRFMTC(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("DBCTLRFMTC",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordDBCTLRFMTC.copy: DBCTLRFMTC
 ;
 Q $$copy^UCGMR(this)
