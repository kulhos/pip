 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordCTBLINST ****
 ; 
 ; 02/24/2010 18:38 - pip
 ; 
 ;
 ; Record Class code for table CTBLINST
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordCTBLINST",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordCTBLINST"
 S vobj(vOid)=$G(^CTBL("INST",v1))
 I vobj(vOid)="",'($D(^CTBL("INST",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,CTBLINST",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 I ctblinst="",'($D(^CTBL("INST",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,CTBLINST",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q ctblinst
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^CTBL("INST",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,ctblinst,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("%NODE,CIDPP,INNUM,KEY,LOG",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="KEY:T:"
 S map(-1)="%NODE:T:5;CIDPP:T:3;INNUM:N:1;LOG:T:2"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N ctblinst S ctblinst=$$vRCgetRecord0^RecordCTBLINST(KEY,0)
 .	S vobj(ctblinst,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordCTBLINST(ctblinst,$$initPar^UCUTILN(params)) K vobj(ctblinst,-100) S vobj(ctblinst,-2)=1 TC:vTp  
 .	K vobj(+$G(ctblinst)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^CTBL("INST",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^CTBL("INST",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^CTBL("INST",vobj(this,-3))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("INNUM")
...
vRCchkReqForUpdate(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("KEY")
 I ($D(vobj(this,-100,"0*","INNUM"))&($P($E($G(vobj(this,-100,"0*","INNUM")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))),($P(vobj(this),$C(124),1)="") D vRCrequiredErr("INNUM")
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("CTBLINST","MSG",1767,"CTBLINST."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I ($L(vobj(this,-3))>10) D vRCvalidateDDerr("KEY",$$^MSG(1076,10))
 I ($L($P(vobj(this),$C(124),5))>20) D vRCvalidateDDerr("%NODE",$$^MSG(1076,20))
 I ($L($P(vobj(this),$C(124),3))>50) D vRCvalidateDDerr("CIDPP",$$^MSG(1076,50))
 I '($P(vobj(this),$C(124),1)=""),'(+$P(vobj(this),$C(124),1)=$P(vobj(this),$C(124),1))  S $P(vobj(this),$C(124),1)=$$vRCtrimNumber($P(vobj(this),$C(124),1))
 S X=$P(vobj(this),$C(124),1) I '(X=""),(X'?1.5N),(X'?1"-"1.4N) D vRCvalidateDDerr("INNUM",$$^MSG(742,"N"))
 I ($L($P(vobj(this),$C(124),2))>12) D vRCvalidateDDerr("LOG",$$^MSG(1076,12))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","KEY"))&($P($E($G(vobj(this,-100,"1*","KEY")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>10) D vRCvalidateDDerr("KEY",$$^MSG(1076,10))
 I ($D(vobj(this,-100,"0*","%NODE"))&($P($E($G(vobj(this,-100,"0*","%NODE")),5,9999),$C(124))'=$P(vobj(this),$C(124),5))) I ($L($P(vobj(this),$C(124),5))>20) D vRCvalidateDDerr("%NODE",$$^MSG(1076,20))
 I ($D(vobj(this,-100,"0*","CIDPP"))&($P($E($G(vobj(this,-100,"0*","CIDPP")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I ($L($P(vobj(this),$C(124),3))>50) D vRCvalidateDDerr("CIDPP",$$^MSG(1076,50))
 I ($D(vobj(this,-100,"0*","INNUM"))&($P($E($G(vobj(this,-100,"0*","INNUM")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))),'($P(vobj(this),$C(124),1)=""),'(+$P(vobj(this),$C(124),1)=$P(vobj(this),$C(124),1))  S $P(vobj(this),$C(124),1)=$$vRCtrimNumber($P(vobj(this),$C(124),1))
 I ($D(vobj(this,-100,"0*","INNUM"))&($P($E($G(vobj(this,-100,"0*","INNUM")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) S X=$P(vobj(this),$C(124),1) I '(X=""),(X'?1.5N),(X'?1"-"1.4N) D vRCvalidateDDerr("INNUM",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","LOG"))&($P($E($G(vobj(this,-100,"0*","LOG")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>12) D vRCvalidateDDerr("LOG",$$^MSG(1076,12))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("CTBLINST","MSG",979,"CTBLINST."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","KEY")):$P($E(vobj(this,-100,"1*","KEY"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^CTBL("INST",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordCTBLINST(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("CTBLINST",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordCTBLINST.copy: CTBLINST
 ;
 Q $$copy^UCGMR(this)
