 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordTRAP0 ****
 ; 
 ; 02/24/2010 18:41 - pip
 ; 
 ;
 ; Record Class code for table TRAP0
 ;
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 I vobj(vOid)="",'($D(^TRAP(v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,TRAP0",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 I trap0="",'($D(^TRAP(v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,TRAP0",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q trap0
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","PID"))&($P($E($G(vobj(this,-100,"1*","PID")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","SEQ"))&($P($E($G(vobj(this,-100,"2*","SEQ")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^TRAP(V1,V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,trap0,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DATA,PID,SEQ",",",$char(9),0,0,"")
 ;
columnListBM() ; 
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N trap0 S trap0=$$vRCgetRecord0^RecordTRAP0(PID,SEQ,0)
 .	S vobj(trap0,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordTRAP0(trap0,$$initPar^UCUTILN(params)) K vobj(trap0,-100) S vobj(trap0,-2)=1 TC:vTp  
 .	K vobj(+$G(trap0)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^TRAP(V1,V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) K:$D(vobj(this,1,1)) ^TRAP(vobj(this,-3),vobj(this,-4)) S ^TRAP(vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	;    #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; GROUP=BYPASS
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this,1,1)) N vS1,vS2 S vS1=0 F vS2=1:450:$ZL(vobj(this,1,1)) S vS1=vS1+1,^TRAP(vobj(this,-3),vobj(this,-4),vS1)=$ZE(vobj(this,1,1),vS2,vS2+449)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 kill ^TRAP(vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("TRAP0","MSG",1767,"TRAP0."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DATA",$$^MSG(1076,40))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"0*","DATA"))&($P($E($G(vobj(this,-100,"0*","DATA")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DATA",$$^MSG(1076,40))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("TRAP0","MSG",979,"TRAP0."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","SEQ")):$P($E(vobj(this,-100,"2*","SEQ"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^TRAP(V1,V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordTRAP0(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("TRAP0",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordTRAP0.copy: TRAP0
 ;
 N vNod,vOid
 I $G(vobj(v1,-2)) D
 . N von,vol S von="",vol=$D(vobj(v1,1,1)) S:'vol vobj(v1,1,1)="" F  Q:vol  S von=$O(^TRAP(vobj(v1,-3),vobj(v1,-4),von)) quit:von=""  S vobj(v1,1,1)=vobj(v1,1,1)_^TRAP(vobj(v1,-3),vobj(v1,-4),von)
