 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordCTBLMTM ****
 ; 
 ; 02/24/2010 18:38 - pip
 ; 
 ;
 ; Record Class code for table CTBLMTM
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordCTBLMTM",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 I vobj(vOid)="",'($D(^CTBL("MTM",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,CTBLMTM",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 I ctblmtm="",'($D(^CTBL("MTM",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,CTBLMTM",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q ctblmtm
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","MTMID"))&($P($E($G(vobj(this,-100,"1*","MTMID")),5,9999),$C(124))'=vobj(this,-3))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^CTBL("MTM",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,ctblmtm,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DESC,MTMID,STARTUP",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="MTMID:U:"
 S map(-1)="DESC:T:1;STARTUP:T:2"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N ctblmtm S ctblmtm=$$vRCgetRecord0^RecordCTBLMTM(MTMID,0)
 .	S vobj(ctblmtm,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordCTBLMTM(ctblmtm,$$initPar^UCUTILN(params)) K vobj(ctblmtm,-100) S vobj(ctblmtm,-2)=1 TC:vTp  
 .	K vobj(+$G(ctblmtm)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^CTBL("MTM",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^CTBL("MTM",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^CTBL("MTM",vobj(this,-3))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("DESC")
 I (vobj(this,-3)="") D vRCrequiredErr("MTMID")
 I ($P(vobj(this),$C(124),2)="") D vRCrequiredErr("STARTUP")
 Q 
 ;
vRCchkReqForUpdate(this) ; 
 ...
 I ($D(vobj(this,-100,"0*","STARTUP"))&($P($E($G(vobj(this,-100,"0*","STARTUP")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))),($P(vobj(this),$C(124),2)="") D vRCrequiredErr("STARTUP")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("CTBLMTM","MSG",1767,"CTBLMTM."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 S X=vobj(this,-3) I '(X="") S errmsg=$$VAL^DBSVER("U",7,1,,"X?1.7AN") I '(errmsg="") D vRCvalidateDDerr("MTMID",$$^MSG(979,"CTBLMTM.MTMID"_" "_errmsg))
 I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DESC",$$^MSG(1076,40))
 I ($L($P(vobj(this),$C(124),2))>60) D vRCvalidateDDerr("STARTUP",$$^MSG(1076,60))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","MTMID"))&($P($E($G(vobj(this,-100,"1*","MTMID")),5,9999),$C(124))'=vobj(this,-3))) S X=vobj(this,-3) I '(X="") S errmsg=$$VAL^DBSVER("U",7,1,,"X?1.7AN") I '(errmsg="") D vRCvalidateDDerr("MTMID",$$^MSG(979,"CTBLMTM.MTMID"_" "_errmsg))
 ...
 I ($D(vobj(this,-100,"0*","STARTUP"))&($P($E($G(vobj(this,-100,"0*","STARTUP")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>60) D vRCvalidateDDerr("STARTUP",$$^MSG(1076,60))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("CTBLMTM","MSG",979,"CTBLMTM."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","MTMID")):$P($E(vobj(this,-100,"1*","MTMID"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^CTBL("MTM",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordCTBLMTM(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("CTBLMTM",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordCTBLMTM.copy: CTBLMTM
 ;
 Q $$copy^UCGMR(this)
