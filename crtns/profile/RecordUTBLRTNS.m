 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordUTBLRTNS ****
 ; 
 ; 02/24/2010 18:42 - pip
 ; 
 ;
 ; Record Class code for table UTBLRTNS
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLRTNS",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLRTNS"
 S vobj(vOid)=$G(^UTBL("RTNS",v1))
 I vobj(vOid)="",'($D(^UTBL("RTNS",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLRTNS",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S utblrtns=$G(^UTBL("RTNS",v1))
 I utblrtns="",'($D(^UTBL("RTNS",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLRTNS",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q utblrtns
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","ROUTINE"))&($P($E($G(vobj(this,-100,"1*","ROUTINE")),5,9999),$C(124))'=vobj(this,-3))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^UTBL("RTNS",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,utblrtns,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("ROUTINE,VALID24X7",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="ROUTINE:T:"
 S map(-1)="VALID24X7:L:1"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N utblrtns S utblrtns=$$vRCgetRecord0^RecordUTBLRTNS(ROUTINE,0)
 .	S vobj(utblrtns,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLRTNS(utblrtns,$$initPar^UCUTILN(params)) K vobj(utblrtns,-100) S vobj(utblrtns,-2)=1 TC:vTp  
 .	K vobj(+$G(utblrtns)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^UTBL("RTNS",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^UTBL("RTNS",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
...
vRCchkReqForInsert(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("ROUTINE")
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("VALID24X7")
 Q 
 ;
vRCchkReqForUpdate(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("ROUTINE")
 I ($D(vobj(this,-100,"0*","VALID24X7"))&($P($E($G(vobj(this,-100,"0*","VALID24X7")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))),($P(vobj(this),$C(124),1)="") D vRCrequiredErr("VALID24X7")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLRTNS","MSG",1767,"UTBLRTNS."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I ($L(vobj(this,-3))>21) D vRCvalidateDDerr("ROUTINE",$$^MSG(1076,21))
 I '(($P(vobj(this),$C(124),1)=1)!($P(vobj(this),$C(124),1)=0)) D vRCvalidateDDerr("VALID24X7",$$^MSG(742,"L"))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","ROUTINE"))&($P($E($G(vobj(this,-100,"1*","ROUTINE")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>21) D vRCvalidateDDerr("ROUTINE",$$^MSG(1076,21))
 I ($D(vobj(this,-100,"0*","VALID24X7"))&($P($E($G(vobj(this,-100,"0*","VALID24X7")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I '(($P(vobj(this),$C(124),1)=1)!($P(vobj(this),$C(124),1)=0)) D vRCvalidateDDerr("VALID24X7",$$^MSG(742,"L"))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLRTNS","MSG",979,"UTBLRTNS."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","ROUTINE")):$P($E(vobj(this,-100,"1*","ROUTINE"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^UTBL("RTNS",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLRTNS(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("UTBLRTNS",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordUTBLRTNS.copy: UTBLRTNS
 ;
 Q $$copy^UCGMR(this)
