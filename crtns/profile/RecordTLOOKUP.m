 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordTLOOKUP ****
 ; 
 ; 02/24/2010 18:41 - pip
 ; 
 ;
 ; Record Class code for table TLOOKUP
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordTLOOKUP",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordTLOOKUP"
 S vobj(vOid)=$G(^TMP(v1,v2))
 I vobj(vOid)="",'($D(^TMP(v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,TLOOKUP",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S tlookup=$G(^TMP(v1,v2))
 I tlookup="",'($D(^TMP(v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,TLOOKUP",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q tlookup
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","PID"))&($P($E($G(vobj(this,-100,"1*","PID")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","TNAME"))&($P($E($G(vobj(this,-100,"2*","TNAME")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,tlookup,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DESC,FILE,PID,TNAME",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ...
 S map(-4)="TNAME:T:"
 S map(-3)="PID:T:"
 S map(-1)="DESC:T:1;FILE:U:2"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N tlookup S tlookup=$$vRCgetRecord0^RecordTLOOKUP(PID,TNAME,0)
 .	S vobj(tlookup,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordTLOOKUP(tlookup,$$initPar^UCUTILN(params)) K vobj(tlookup,-100) S vobj(tlookup,-2)=1 TC:vTp  
 .	K vobj(+$G(tlookup)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCchkReqForInsert(this) ; 
 I (vobj(this,-4)="") D vRCrequiredErr("TNAME")
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("TLOOKUP","MSG",1767,"TLOOKUP."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I ($L(vobj(this,-3))>20) D vRCvalidateDDerr("PID",$$^MSG(1076,20))
 I ($L(vobj(this,-4))>12) D vRCvalidateDDerr("TNAME",$$^MSG(1076,12))
 I ($L($P(vobj(this),$C(124),1))>80) D vRCvalidateDDerr("DESC",$$^MSG(1076,80))
 I ($P(vobj(this),$C(124),2)'=$ZCONVERT($P(vobj(this),$C(124),2),"U")) D vRCvalidateDDerr("FILE",$$^MSG(1476))
 I ($L($P(vobj(this),$C(124),2))>20) D vRCvalidateDDerr("FILE",$$^MSG(1076,20))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","PID"))&($P($E($G(vobj(this,-100,"1*","PID")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>20) D vRCvalidateDDerr("PID",$$^MSG(1076,20))
 I ($D(vobj(this,-100,"2*","TNAME"))&($P($E($G(vobj(this,-100,"2*","TNAME")),5,9999),$C(124))'=vobj(this,-4))) I ($L(vobj(this,-4))>12) D vRCvalidateDDerr("TNAME",$$^MSG(1076,12))
 I ($D(vobj(this,-100,"0*","DESC"))&($P($E($G(vobj(this,-100,"0*","DESC")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>80) D vRCvalidateDDerr("DESC",$$^MSG(1076,80))
 I ($P(vobj(this),$C(124),2)'=$ZCONVERT($P(vobj(this),$C(124),2),"U")) D vRCvalidateDDerr("FILE",$$^MSG(1476))
 I ($D(vobj(this,-100,"0*","FILE"))&($P($E($G(vobj(this,-100,"0*","FILE")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>20) D vRCvalidateDDerr("FILE",$$^MSG(1076,20))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("TLOOKUP","MSG",979,"TLOOKUP."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","TNAME")):$P($E(vobj(this,-100,"2*","TNAME"),5,9999),$C(124)),1:vobj(this,-4))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordTLOOKUP(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("TLOOKUP",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordTLOOKUP.copy: TLOOKUP
 ;
 Q $$copy^UCGMR(this)
