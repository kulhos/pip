 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordTBLINT ****
 ; 
 ; 02/24/2010 18:41 - pip
 ; 
 ;
 ; Record Class code for table TBLINT
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordTBLINT",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordTBLINT"
 S vobj(vOid)=$G(^TMP(v1,v2))
 I vobj(vOid)="",'($D(^TMP(v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,TBLINT",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S tblint=$G(^TMP(v1,v2))
 I tblint="",'($D(^TMP(v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,TBLINT",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q tblint
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","PID"))&($P($E($G(vobj(this,-100,"1*","PID")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","FILE"))&($P($E($G(vobj(this,-100,"2*","FILE")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^TMP(V1,V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,tblint,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("ACCKEY,ACCKEY1,FILE,FILE1,FILE2,PID",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ...
 S map(-4)="FILE:N:"
 S map(-3)="PID:N:"
 S map(-1)="ACCKEY:T:4;ACCKEY1:T:2;FILE1:T:1;FILE2:T:3"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N tblint S tblint=$$vRCgetRecord0^RecordTBLINT(PID,FILE,0)
 .	S vobj(tblint,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordTBLINT(tblint,$$initPar^UCUTILN(params)) K vobj(tblint,-100) S vobj(tblint,-2)=1 TC:vTp  
 .	K vobj(+$G(tblint)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^TMP(V1,V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^TMP(vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^TMP(vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I (vobj(this,-4)="") D vRCrequiredErr("FILE")
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("TBLINT","MSG",1767,"TBLINT."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 S X=vobj(this,-4) I '(X=""),(X'?1.12N),(X'?1"-"1.11N) D vRCvalidateDDerr("FILE",$$^MSG(742,"N"))
 I ($L($P(vobj(this),$C(124),4))>30) D vRCvalidateDDerr("ACCKEY",$$^MSG(1076,30))
 I ($L($P(vobj(this),$C(124),2))>30) D vRCvalidateDDerr("ACCKEY1",$$^MSG(1076,30))
 I ($L($P(vobj(this),$C(124),1))>12) D vRCvalidateDDerr("FILE1",$$^MSG(1076,12))
 I ($L($P(vobj(this),$C(124),3))>12) D vRCvalidateDDerr("FILE2",$$^MSG(1076,12))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"2*","FILE"))&($P($E($G(vobj(this,-100,"2*","FILE")),5,9999),$C(124))'=vobj(this,-4))),'(vobj(this,-4)=""),'(+vobj(this,-4)=vobj(this,-4))  S vobj(this,-4)=$$vRCtrimNumber(vobj(this,-4))
 I ($D(vobj(this,-100,"2*","FILE"))&($P($E($G(vobj(this,-100,"2*","FILE")),5,9999),$C(124))'=vobj(this,-4))) S X=vobj(this,-4) I '(X=""),(X'?1.12N),(X'?1"-"1.11N) D vRCvalidateDDerr("FILE",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","ACCKEY"))&($P($E($G(vobj(this,-100,"0*","ACCKEY")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) I ($L($P(vobj(this),$C(124),4))>30) D vRCvalidateDDerr("ACCKEY",$$^MSG(1076,30))
 I ($D(vobj(this,-100,"0*","ACCKEY1"))&($P($E($G(vobj(this,-100,"0*","ACCKEY1")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>30) D vRCvalidateDDerr("ACCKEY1",$$^MSG(1076,30))
 I ($D(vobj(this,-100,"0*","FILE1"))&($P($E($G(vobj(this,-100,"0*","FILE1")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>12) D vRCvalidateDDerr("FILE1",$$^MSG(1076,12))
 I ($D(vobj(this,-100,"0*","FILE2"))&($P($E($G(vobj(this,-100,"0*","FILE2")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I ($L($P(vobj(this),$C(124),3))>12) D vRCvalidateDDerr("FILE2",$$^MSG(1076,12))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("TBLINT","MSG",979,"TBLINT."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","FILE")):$P($E(vobj(this,-100,"2*","FILE"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^TMP(V1,V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordTBLINT(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("TBLINT",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordTBLINT.copy: TBLINT
 ;
 Q $$copy^UCGMR(this)
