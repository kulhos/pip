 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordUTBLUTBL ****
 ; 
 ; 02/24/2010 18:42 - pip
 ; 
 ;
 ; Record Class code for table UTBLUTBL
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLUTBL",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLUTBL"
 S vobj(vOid)=$G(^UTBL("UTBL",v1,v2))
 I vobj(vOid)="",'($D(^UTBL("UTBL",v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLUTBL",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S utblutbl=$G(^UTBL("UTBL",v1,v2))
 I utblutbl="",'($D(^UTBL("UTBL",v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLUTBL",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q utblutbl
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","TBL"))&($P($E($G(vobj(this,-100,"1*","TBL")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","KEY"))&($P($E($G(vobj(this,-100,"2*","KEY")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^UTBL("UTBL",V1,V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,utblutbl,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("KEY,RWD,TBL",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N utblutbl S utblutbl=$$vRCgetRecord0^RecordUTBLUTBL(TBL,KEY,0)
 .	S vobj(utblutbl,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLUTBL(utblutbl,$$initPar^UCUTILN(params)) K vobj(utblutbl,-100) S vobj(utblutbl,-2)=1 TC:vTp  
 .	K vobj(+$G(utblutbl)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^UTBL("UTBL",V1,V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^UTBL("UTBL",vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^UTBL("UTBL",vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I (vobj(this,-4)="") D vRCrequiredErr("KEY")
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("RWD")
 I (vobj(this,-3)="") D vRCrequiredErr("TBL")
 Q 
 ;
vRCchkReqForUpdate(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("TBL")
 I (vobj(this,-4)="") D vRCrequiredErr("KEY")
 I ($D(vobj(this,-100,"0*","RWD"))&($P($E($G(vobj(this,-100,"0*","RWD")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))),($P(vobj(this),$C(124),1)="") D vRCrequiredErr("RWD")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLUTBL","MSG",1767,"UTBLUTBL."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I ($L(vobj(this,-3))>12) D vRCvalidateDDerr("TBL",$$^MSG(1076,12))
 S X=vobj(this,-4) I '(X=""),'($D(^SCAU(0,X))#2) D vRCvalidateDDerr("KEY",$$^MSG(1485,X))
 I ($P(vobj(this),$C(124),1)'=$ZCONVERT($P(vobj(this),$C(124),1),"U")) D vRCvalidateDDerr("RWD",$$^MSG(1476))
 I ($L($P(vobj(this),$C(124),1))>3) D vRCvalidateDDerr("RWD",$$^MSG(1076,3))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","TBL"))&($P($E($G(vobj(this,-100,"1*","TBL")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>12) D vRCvalidateDDerr("TBL",$$^MSG(1076,12))
 I ($D(vobj(this,-100,"2*","KEY"))&($P($E($G(vobj(this,-100,"2*","KEY")),5,9999),$C(124))'=vobj(this,-4))) S X=vobj(this,-4) I '(X=""),'($D(^SCAU(0,X))#2) D vRCvalidateDDerr("KEY",$$^MSG(1485,X))
 I ($P(vobj(this),$C(124),1)'=$ZCONVERT($P(vobj(this),$C(124),1),"U")) D vRCvalidateDDerr("RWD",$$^MSG(1476))
 I ($D(vobj(this,-100,"0*","RWD"))&($P($E($G(vobj(this,-100,"0*","RWD")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>3) D vRCvalidateDDerr("RWD",$$^MSG(1076,3))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLUTBL","MSG",979,"UTBLUTBL."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","TBL")):$P($E(vobj(this,-100,"1*","TBL"),5,9999),$C(124)),1:vobj(this,-3))
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","KEY")):$P($E(vobj(this,-100,"2*","KEY"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^UTBL("UTBL",V1,V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLUTBL(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("UTBLUTBL",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordUTBLUTBL.copy: UTBLUTBL
 ;
 Q $$copy^UCGMR(this)
