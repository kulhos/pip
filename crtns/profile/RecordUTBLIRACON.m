 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordUTBLIRACON ****
 ; 
 ; 02/24/2010 18:42 - pip
 ; 
 ;
 ; Record Class code for table UTBLIRACON
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLIRACON",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLIRACON"
 S vobj(vOid)=$G(^UTBL("IRACON",v1))
 I vobj(vOid)="",'($D(^UTBL("IRACON",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLIRACON",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S utbliracon=$G(^UTBL("IRACON",v1))
 I utbliracon="",'($D(^UTBL("IRACON",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLIRACON",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q utbliracon
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^UTBL("IRACON",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,utbliracon,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("AGE,DEPNOT,DESC,KEY,LIF,LOCK",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="KEY:N:"
 S map(-1)="AGE:L:3;DEPNOT:L:4;DESC:T:1;LIF:L:6;LOCK:L:7"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N utbliracon S utbliracon=$$vRCgetRecord0^RecordUTBLIRACON(KEY,0)
 .	S vobj(utbliracon,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLIRACON(utbliracon,$$initPar^UCUTILN(params)) K vobj(utbliracon,-100) S vobj(utbliracon,-2)=1 TC:vTp  
 .	K vobj(+$G(utbliracon)) Q 
 Q 
 ;
vhasLiterals() ; 
 Q 0
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^UTBL("IRACON",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^UTBL("IRACON",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
...
vRCchkReqForInsert(this) ; 
 I ($P(vobj(this),$C(124),3)="") D vRCrequiredErr("AGE")
 I ($P(vobj(this),$C(124),4)="") D vRCrequiredErr("DEPNOT")
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("DESC")
 I (vobj(this,-3)="") D vRCrequiredErr("KEY")
 I ($P(vobj(this),$C(124),6)="") D vRCrequiredErr("LIF")
 I ($P(vobj(this),$C(124),7)="") D vRCrequiredErr("LOCK")
 Q 
 ;
vRCchkReqForUpdate(this) ; 
 I (vobj(this,-3)="") D vRCrequiredErr("KEY")
 I ($D(vobj(this,-100,"0*","AGE"))&($P($E($G(vobj(this,-100,"0*","AGE")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))),($P(vobj(this),$C(124),3)="") D vRCrequiredErr("AGE")
 I ($D(vobj(this,-100,"0*","DEPNOT"))&($P($E($G(vobj(this,-100,"0*","DEPNOT")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))),($P(vobj(this),$C(124),4)="") D vRCrequiredErr("DEPNOT")
 ...
 I ($D(vobj(this,-100,"0*","LIF"))&($P($E($G(vobj(this,-100,"0*","LIF")),5,9999),$C(124))'=$P(vobj(this),$C(124),6))),($P(vobj(this),$C(124),6)="") D vRCrequiredErr("LIF")
 I ($D(vobj(this,-100,"0*","LOCK"))&($P($E($G(vobj(this,-100,"0*","LOCK")),5,9999),$C(124))'=$P(vobj(this),$C(124),7))),($P(vobj(this),$C(124),7)="") D vRCrequiredErr("LOCK")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLIRACON","MSG",1767,"UTBLIRACON."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 S X=vobj(this,-3) I '(X=""),(X'?1.10N),(X'?1"-"1.9N) D vRCvalidateDDerr("KEY",$$^MSG(742,"N"))
 I '(($P(vobj(this),$C(124),3)=1)!($P(vobj(this),$C(124),3)=0)) D vRCvalidateDDerr("AGE",$$^MSG(742,"L"))
 I '(($P(vobj(this),$C(124),4)=1)!($P(vobj(this),$C(124),4)=0)) D vRCvalidateDDerr("DEPNOT",$$^MSG(742,"L"))
 I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DESC",$$^MSG(1076,40))
 I '(($P(vobj(this),$C(124),6)=1)!($P(vobj(this),$C(124),6)=0)) D vRCvalidateDDerr("LIF",$$^MSG(742,"L"))
 I '(($P(vobj(this),$C(124),7)=1)!($P(vobj(this),$C(124),7)=0)) D vRCvalidateDDerr("LOCK",$$^MSG(742,"L"))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","KEY"))&($P($E($G(vobj(this,-100,"1*","KEY")),5,9999),$C(124))'=vobj(this,-3))),'(vobj(this,-3)=""),'(+vobj(this,-3)=vobj(this,-3))  S vobj(this,-3)=$$vRCtrimNumber(vobj(this,-3))
 I ($D(vobj(this,-100,"1*","KEY"))&($P($E($G(vobj(this,-100,"1*","KEY")),5,9999),$C(124))'=vobj(this,-3))) S X=vobj(this,-3) I '(X=""),(X'?1.10N),(X'?1"-"1.9N) D vRCvalidateDDerr("KEY",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","AGE"))&($P($E($G(vobj(this,-100,"0*","AGE")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I '(($P(vobj(this),$C(124),3)=1)!($P(vobj(this),$C(124),3)=0)) D vRCvalidateDDerr("AGE",$$^MSG(742,"L"))
 I ($D(vobj(this,-100,"0*","DEPNOT"))&($P($E($G(vobj(this,-100,"0*","DEPNOT")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) I '(($P(vobj(this),$C(124),4)=1)!($P(vobj(this),$C(124),4)=0)) D vRCvalidateDDerr("DEPNOT",$$^MSG(742,"L"))
 ...
 I ($D(vobj(this,-100,"0*","LIF"))&($P($E($G(vobj(this,-100,"0*","LIF")),5,9999),$C(124))'=$P(vobj(this),$C(124),6))) I '(($P(vobj(this),$C(124),6)=1)!($P(vobj(this),$C(124),6)=0)) D vRCvalidateDDerr("LIF",$$^MSG(742,"L"))
 I ($D(vobj(this,-100,"0*","LOCK"))&($P($E($G(vobj(this,-100,"0*","LOCK")),5,9999),$C(124))'=$P(vobj(this),$C(124),7))) I '(($P(vobj(this),$C(124),7)=1)!($P(vobj(this),$C(124),7)=0)) D vRCvalidateDDerr("LOCK",$$^MSG(742,"L"))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLIRACON","MSG",979,"UTBLIRACON."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","KEY")):$P($E(vobj(this,-100,"1*","KEY"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^UTBL("IRACON",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLIRACON(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("UTBLIRACON",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordUTBLIRACON.copy: UTBLIRACON
 ;
 Q $$copy^UCGMR(this)
