 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordUTBLPRODRLDT ****
 ; 
 ; 02/24/2010 18:42 - pip
 ; 
 ;
 ; Record Class code for table UTBLPRODRLDT
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLPRODRLDT",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 I vobj(vOid)="",'($D(^UTBL("PRODRL",v1,v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLPRODRLDT",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S utblprodrldt=$G(^UTBL("PRODRL",v1,v2))
 I utblprodrldt="",'($D(^UTBL("PRODRL",v1,v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLPRODRLDT",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q utblprodrldt
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","RULEID"))&($P($E($G(vobj(this,-100,"1*","RULEID")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","DECISION"))&($P($E($G(vobj(this,-100,"2*","DECISION")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^UTBL("PRODRL",V1,V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,utblprodrldt,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DECISION,LDATE,LTIME,LUSER,RULEID,TEST",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ...
 S map(-4)="DECISION:N:"
 S map(-3)="RULEID:N:"
 S map(-1)="LDATE:D:3;LTIME:C:4;LUSER:T:2;TEST:T:1"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N utblprodrldt S utblprodrldt=$$vRCgetRecord0^RecordUTBLPRODRLDT(RULEID,DECISION,0)
 .	S vobj(utblprodrldt,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLPRODRLDT(utblprodrldt,$$initPar^UCUTILN(params)) K vobj(utblprodrldt,-100) S vobj(utblprodrldt,-2)=1 TC:vTp  
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^UTBL("PRODRL",V1,V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .  S $P(vobj(this),$C(124),3)=TJD
 .  S $P(vobj(this),$C(124),4)=$P($H,",",2)
 .  S $P(vobj(this),$C(124),2)=$E(%UID,1,20)
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^UTBL("PRODRL",vobj(this,-3),vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^UTBL("PRODRL",vobj(this,-3),vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I (vobj(this,-4)="") D vRCrequiredErr("DECISION")
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLPRODRLDT","MSG",1767,"UTBLPRODRLDT."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 S X=vobj(this,-4) I '(X=""),(X'?1.4N),(X'?1"-"1.3N) D vRCvalidateDDerr("DECISION",$$^MSG(742,"N"))
 S X=$P(vobj(this),$C(124),3) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LDATE",$$^MSG(742,"D"))
 S X=$P(vobj(this),$C(124),4) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LTIME",$$^MSG(742,"C"))
 I ($L($P(vobj(this),$C(124),2))>20) D vRCvalidateDDerr("LUSER",$$^MSG(1076,20))
 I ($L($P(vobj(this),$C(124),1))>250) D vRCvalidateDDerr("TEST",$$^MSG(1076,250))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"2*","DECISION"))&($P($E($G(vobj(this,-100,"2*","DECISION")),5,9999),$C(124))'=vobj(this,-4))),'(vobj(this,-4)=""),'(+vobj(this,-4)=vobj(this,-4))  S vobj(this,-4)=$$vRCtrimNumber(vobj(this,-4))
 I ($D(vobj(this,-100,"2*","DECISION"))&($P($E($G(vobj(this,-100,"2*","DECISION")),5,9999),$C(124))'=vobj(this,-4))) S X=vobj(this,-4) I '(X=""),(X'?1.4N),(X'?1"-"1.3N) D vRCvalidateDDerr("DECISION",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","LDATE"))&($P($E($G(vobj(this,-100,"0*","LDATE")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) S X=$P(vobj(this),$C(124),3) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LDATE",$$^MSG(742,"D"))
 I ($D(vobj(this,-100,"0*","LTIME"))&($P($E($G(vobj(this,-100,"0*","LTIME")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) S X=$P(vobj(this),$C(124),4) I '(X=""),(X'?1.5N) D vRCvalidateDDerr("LTIME",$$^MSG(742,"C"))
 I ($D(vobj(this,-100,"0*","LUSER"))&($P($E($G(vobj(this,-100,"0*","LUSER")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>20) D vRCvalidateDDerr("LUSER",$$^MSG(1076,20))
 I ($D(vobj(this,-100,"0*","TEST"))&($P($E($G(vobj(this,-100,"0*","TEST")),5,9999),$C(124))'=$P(vobj(this),$C(124),1))) I ($L($P(vobj(this),$C(124),1))>250) D vRCvalidateDDerr("TEST",$$^MSG(1076,250))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLPRODRLDT","MSG",979,"UTBLPRODRLDT."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","DECISION")):$P($E(vobj(this,-100,"2*","DECISION"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^UTBL("PRODRL",V1,V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLPRODRLDT(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("UTBLPRODRLDT",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
