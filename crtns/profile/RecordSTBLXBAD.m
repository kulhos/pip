 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordSTBLXBAD ****
 ; 
 ; 02/24/2010 18:41 - pip
 ; 
 ;
 ; Record Class code for table STBLXBAD
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordSTBLXBAD",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordSTBLXBAD"
 S vobj(vOid)=$G(^STBL("XBAD",v1))
 I vobj(vOid)="",'($D(^STBL("XBAD",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,STBLXBAD",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S stblxbad=$G(^STBL("XBAD",v1))
 I stblxbad="",'($D(^STBL("XBAD",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,STBLXBAD",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q stblxbad
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^STBL("XBAD",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,stblxbad,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DESC,FLAG,FORMAT,KEY,NODE,PIECE",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="KEY:T:"
 S map(-1)="DESC:T:1;FLAG:N:4;FORMAT:T:5;NODE:N:2;PIECE:T:3"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N stblxbad S stblxbad=$$vRCgetRecord0^RecordSTBLXBAD(KEY,0)
 .	S vobj(stblxbad,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSTBLXBAD(stblxbad,$$initPar^UCUTILN(params)) K vobj(stblxbad,-100) S vobj(stblxbad,-2)=1 TC:vTp  
 .	K vobj(+$G(stblxbad)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^STBL("XBAD",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^STBL("XBAD",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^STBL("XBAD",vobj(this,-3))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("DESC")
 I ($P(vobj(this),$C(124),4)="") D vRCrequiredErr("FLAG")
...
vRCchkReqForUpdate(this) ; 
 ...
 I ($D(vobj(this,-100,"0*","FLAG"))&($P($E($G(vobj(this,-100,"0*","FLAG")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))),($P(vobj(this),$C(124),4)="") D vRCrequiredErr("FLAG")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("STBLXBAD","MSG",1767,"STBLXBAD."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 S X=$P(vobj(this),$C(124),4) I '(X=""),(X'?1N) D vRCvalidateDDerr("FLAG",$$^MSG(742,"N"))
 I ($L($P(vobj(this),$C(124),5))>1) D vRCvalidateDDerr("FORMAT",$$^MSG(1076,1))
 I '($P(vobj(this),$C(124),2)=""),'(+$P(vobj(this),$C(124),2)=$P(vobj(this),$C(124),2))  S $P(vobj(this),$C(124),2)=$$vRCtrimNumber($P(vobj(this),$C(124),2))
 S X=$P(vobj(this),$C(124),2) I '(X=""),(X'?1.4N),(X'?1"-"1.3N) D vRCvalidateDDerr("NODE",$$^MSG(742,"N"))
 I ($L($P(vobj(this),$C(124),3))>4) D vRCvalidateDDerr("PIECE",$$^MSG(1076,4))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"0*","FLAG"))&($P($E($G(vobj(this,-100,"0*","FLAG")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))),'($P(vobj(this),$C(124),4)=""),'(+$P(vobj(this),$C(124),4)=$P(vobj(this),$C(124),4))  S $P(vobj(this),$C(124),4)=$$vRCtrimNumber($P(vobj(this),$C(124),4))
 I ($D(vobj(this,-100,"0*","FLAG"))&($P($E($G(vobj(this,-100,"0*","FLAG")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) S X=$P(vobj(this),$C(124),4) I '(X=""),(X'?1N) D vRCvalidateDDerr("FLAG",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","FORMAT"))&($P($E($G(vobj(this,-100,"0*","FORMAT")),5,9999),$C(124))'=$P(vobj(this),$C(124),5))) I ($L($P(vobj(this),$C(124),5))>1) D vRCvalidateDDerr("FORMAT",$$^MSG(1076,1))
 I ($D(vobj(this,-100,"0*","NODE"))&($P($E($G(vobj(this,-100,"0*","NODE")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))),'($P(vobj(this),$C(124),2)=""),'(+$P(vobj(this),$C(124),2)=$P(vobj(this),$C(124),2))  S $P(vobj(this),$C(124),2)=$$vRCtrimNumber($P(vobj(this),$C(124),2))
 I ($D(vobj(this,-100,"0*","NODE"))&($P($E($G(vobj(this,-100,"0*","NODE")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) S X=$P(vobj(this),$C(124),2) I '(X=""),(X'?1.4N),(X'?1"-"1.3N) D vRCvalidateDDerr("NODE",$$^MSG(742,"N"))
 I ($D(vobj(this,-100,"0*","PIECE"))&($P($E($G(vobj(this,-100,"0*","PIECE")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I ($L($P(vobj(this),$C(124),3))>4) D vRCvalidateDDerr("PIECE",$$^MSG(1076,4))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("STBLXBAD","MSG",979,"STBLXBAD."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","KEY")):$P($E(vobj(this,-100,"1*","KEY"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^STBL("XBAD",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSTBLXBAD(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("STBLXBAD",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordSTBLXBAD.copy: STBLXBAD
 ;
 Q $$copy^UCGMR(this)
