 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordUTBLIRADIS ****
 ; 
 ; 02/24/2010 18:42 - pip
 ; 
 ;
 ; Record Class code for table UTBLIRADIS
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLIRADIS",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordUTBLIRADIS"
 S vobj(vOid)=$G(^UTBL("IRADIS",v1))
 I vobj(vOid)="",'($D(^UTBL("IRADIS",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLIRADIS",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S utbliradis=$G(^UTBL("IRADIS",v1))
 I utbliradis="",'($D(^UTBL("IRADIS",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,UTBLIRADIS",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q utbliradis
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^UTBL("IRADIS",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,utbliradis,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("AGE,DESC,KEY,LIF,PERIOD,WH",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="KEY:N:"
 S map(-1)="AGE:L:3;DESC:T:1;LIF:L:6;PERIOD:L:5;WH:T:4"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N utbliradis S utbliradis=$$vRCgetRecord0^RecordUTBLIRADIS(KEY,0)
 .	S vobj(utbliradis,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLIRADIS(utbliradis,$$initPar^UCUTILN(params)) K vobj(utbliradis,-100) S vobj(utbliradis,-2)=1 TC:vTp  
 .	K vobj(+$G(utbliradis)) Q 
 Q 
 ;
vhasLiterals() ; 
 Q 0
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^UTBL("IRADIS",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^UTBL("IRADIS",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
...
vRCchkReqForInsert(this) ; 
 ...
 I ($P(vobj(this),$C(124),5)="") D vRCrequiredErr("PERIOD")
 I ($P(vobj(this),$C(124),4)="") D vRCrequiredErr("WH")
 Q 
 ;
vRCchkReqForUpdate(this) ; 
 ...
 I ($D(vobj(this,-100,"0*","PERIOD"))&($P($E($G(vobj(this,-100,"0*","PERIOD")),5,9999),$C(124))'=$P(vobj(this),$C(124),5))),($P(vobj(this),$C(124),5)="") D vRCrequiredErr("PERIOD")
 I ($D(vobj(this,-100,"0*","WH"))&($P($E($G(vobj(this,-100,"0*","WH")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))),($P(vobj(this),$C(124),4)="") D vRCrequiredErr("WH")
 Q 
 ;
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLIRADIS","MSG",1767,"UTBLIRADIS."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 I '(($P(vobj(this),$C(124),5)=1)!($P(vobj(this),$C(124),5)=0)) D vRCvalidateDDerr("PERIOD",$$^MSG(742,"L"))
 I ($L($P(vobj(this),$C(124),4))>19) D vRCvalidateDDerr("WH",$$^MSG(1076,19))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"0*","PERIOD"))&($P($E($G(vobj(this,-100,"0*","PERIOD")),5,9999),$C(124))'=$P(vobj(this),$C(124),5))) I '(($P(vobj(this),$C(124),5)=1)!($P(vobj(this),$C(124),5)=0)) D vRCvalidateDDerr("PERIOD",$$^MSG(742,"L"))
 I ($D(vobj(this,-100,"0*","WH"))&($P($E($G(vobj(this,-100,"0*","WH")),5,9999),$C(124))'=$P(vobj(this),$C(124),4))) I ($L($P(vobj(this),$C(124),4))>19) D vRCvalidateDDerr("WH",$$^MSG(1076,19))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("UTBLIRADIS","MSG",979,"UTBLIRADIS."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","KEY")):$P($E(vobj(this,-100,"1*","KEY"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^UTBL("IRADIS",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordUTBLIRADIS(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("UTBLIRADIS",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordUTBLIRADIS.copy: UTBLIRADIS
 ;
 Q $$copy^UCGMR(this)
