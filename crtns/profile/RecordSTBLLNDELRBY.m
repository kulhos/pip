 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordSTBLLNDELRBY ****
 ; 
 ; 02/24/2010 18:41 - pip
 ; 
 ;
 ; Record Class code for table STBLLNDELRBY
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordSTBLLNDELRBY",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordSTBLLNDELRBY"
 S vobj(vOid)=$G(^STBL("LNDELRBY",v1))
 I vobj(vOid)="",'($D(^STBL("LNDELRBY",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,STBLLNDELRBY",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S stbllndelrby=$G(^STBL("LNDELRBY",v1))
 I stbllndelrby="",'($D(^STBL("LNDELRBY",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,STBLLNDELRBY",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q stbllndelrby
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","LNDRBY"))&($P($E($G(vobj(this,-100,"1*","LNDRBY")),5,9999),$C(124))'=vobj(this,-3))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^STBL("LNDELRBY",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,stbllndelrby,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DESC,LNDRBY",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N stbllndelrby S stbllndelrby=$$vRCgetRecord0^RecordSTBLLNDELRBY(LNDRBY,0)
 .	S vobj(stbllndelrby,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSTBLLNDELRBY(stbllndelrby,$$initPar^UCUTILN(params)) K vobj(stbllndelrby,-100) S vobj(stbllndelrby,-2)=1 TC:vTp  
 .	K vobj(+$G(stbllndelrby)) Q 
 Q 
 ;
vhasLiterals() ; 
 Q 0
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^STBL("LNDELRBY",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^STBL("LNDELRBY",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^STBL("LNDELRBY",vobj(this,-3))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I ($P(vobj(this),$C(124),1)="") D vRCrequiredErr("DESC")
 I (vobj(this,-3)="") D vRCrequiredErr("LNDRBY")
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("STBLLNDELRBY","MSG",1767,"STBLLNDELRBY."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I (vobj(this,-3)'=$ZCONVERT(vobj(this,-3),"U")) D vRCvalidateDDerr("LNDRBY",$$^MSG(1476))
 I ($L(vobj(this,-3))>1) D vRCvalidateDDerr("LNDRBY",$$^MSG(1076,1))
...
vRCvalidateDD1(this) ; 
 ...
 N errmsg N X
 I (vobj(this,-3)'=$ZCONVERT(vobj(this,-3),"U")) D vRCvalidateDDerr("LNDRBY",$$^MSG(1476))
 I ($D(vobj(this,-100,"1*","LNDRBY"))&($P($E($G(vobj(this,-100,"1*","LNDRBY")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>1) D vRCvalidateDDerr("LNDRBY",$$^MSG(1076,1))
...
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("STBLLNDELRBY","MSG",979,"STBLLNDELRBY."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","LNDRBY")):$P($E(vobj(this,-100,"1*","LNDRBY"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^STBL("LNDELRBY",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordSTBLLNDELRBY(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("STBLLNDELRBY",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordSTBLLNDELRBY.copy: STBLLNDELRBY
 ;
 Q $$copy^UCGMR(this)
