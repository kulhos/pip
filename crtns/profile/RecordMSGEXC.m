 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordMSGEXC ****
 ; 
 ; 02/24/2010 18:40 - pip
 ; 
 ;
 ; Record Class code for table MSGEXC
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordMSGEXC",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
 S vobj(vOid,-5)=""
...
vRCgetRecord0(v1,v2,v3,vfromDbSet) ; 
 ...
 I vobj(vOid)="",'($D(^MSGEXC(v1,v2,v3))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,MSGEXC",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 S vobj(vOid,-5)=v3
 ;*** End of code by-passed by compiler ***
...
vRCgetRecord0Opt(v1,v2,v3,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S msgexc=$G(^MSGEXC(v1,v2,v3))
 I msgexc="",'($D(^MSGEXC(v1,v2,v3))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,MSGEXC",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q msgexc
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","TOKEN"))&($P($E($G(vobj(this,-100,"1*","TOKEN")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","MSGID"))&($P($E($G(vobj(this,-100,"2*","MSGID")),5,9999),$C(124))'=vobj(this,-4)))!($D(vobj(this,-100,"3*","KEY"))&($P($E($G(vobj(this,-100,"3*","KEY")),5,9999),$C(124))'=vobj(this,-5))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2,V3 S V1=vobj(this,-3),V2=vobj(this,-4),V3=vobj(this,-5) Q:'($D(^MSGEXC(V1,V2,V3))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,msgexc,vkey1,vkey2,vkey3) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("KEY,MSGID,TOKEN",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q $$vStrRep("MESSAGE",",",$char(9),0,0,"")
 ;
columnListCMP() ; 
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N msgexc S msgexc=$$vRCgetRecord0^RecordMSGEXC(TOKEN,MSGID,KEY,0)
 .	S vobj(msgexc,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordMSGEXC(msgexc,$$initPar^UCUTILN(params)) K vobj(msgexc,-100) S vobj(msgexc,-2)=1 TC:vTp  
 .	K vobj(+$G(msgexc)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2,V3 S V1=vobj(this,-3),V2=vobj(this,-4),V3=vobj(this,-5) I '(''($D(^MSGEXC(V1,V2,V3))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) K:$D(vobj(this,1,1)) ^MSGEXC(vobj(this,-3),vobj(this,-4),vobj(this,-5)) S ^MSGEXC(vobj(this,-3),vobj(this,-4),vobj(this,-5))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	;    #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; GROUP=BYPASS
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this,1,1)) N vS1,vS2 S vS1=0 F vS2=1:450:$ZL(vobj(this,1,1)) S vS1=vS1+1,^MSGEXC(vobj(this,-3),vobj(this,-4),vobj(this,-5),vS1)=$ZE(vobj(this,1,1),vS2,vS2+449)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 kill ^MSGEXC(vobj(this,-3),vobj(this,-4),vobj(this,-5))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
vRCchkReqForInsert(this) ; 
 I (vobj(this,-5)="") D vRCrequiredErr("KEY")
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("MSGEXC","MSG",1767,"MSGEXC."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 I ($L(vobj(this,-4))>20) D vRCvalidateDDerr("MSGID",$$^MSG(1076,20))
 I '(vobj(this,-5)=""),'(+vobj(this,-5)=vobj(this,-5))  S vobj(this,-5)=$$vRCtrimNumber(vobj(this,-5))
 S X=vobj(this,-5) I '(X=""),(X'?1.12N),(X'?1"-"1.11N) D vRCvalidateDDerr("KEY",$$^MSG(742,"N"))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 ...
 I ($D(vobj(this,-100,"2*","MSGID"))&($P($E($G(vobj(this,-100,"2*","MSGID")),5,9999),$C(124))'=vobj(this,-4))) I ($L(vobj(this,-4))>20) D vRCvalidateDDerr("MSGID",$$^MSG(1076,20))
 I ($D(vobj(this,-100,"3*","KEY"))&($P($E($G(vobj(this,-100,"3*","KEY")),5,9999),$C(124))'=vobj(this,-5))),'(vobj(this,-5)=""),'(+vobj(this,-5)=vobj(this,-5))  S vobj(this,-5)=$$vRCtrimNumber(vobj(this,-5))
 I ($D(vobj(this,-100,"3*","KEY"))&($P($E($G(vobj(this,-100,"3*","KEY")),5,9999),$C(124))'=vobj(this,-5))) S X=vobj(this,-5) I '(X=""),(X'?1.12N),(X'?1"-"1.11N) D vRCvalidateDDerr("KEY",$$^MSG(742,"N"))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("MSGEXC","MSG",979,"MSGEXC."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey3 S oldKey3=$S($D(vobj(this,-100,"3*","KEY")):$P($E(vobj(this,-100,"3*","KEY"),5,9999),$C(124)),1:vobj(this,-5))
  N V1,V2,V3 S V1=vobj(this,-3),V2=vobj(this,-4),V3=vobj(this,-5) I ($D(^MSGEXC(V1,V2,V3))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordMSGEXC(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("MSGEXC",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vReCp1(v1) ; RecordMSGEXC.copy: MSGEXC
 ...
 N vNod,vOid
 I $G(vobj(v1,-2)) D
 . N von,vol S von="",vol=$D(vobj(v1,1,1)) S:'vol vobj(v1,1,1)="" F  Q:vol  S von=$O(^MSGEXC(vobj(v1,-3),vobj(v1,-4),vobj(v1,-5),von)) quit:von=""  S vobj(v1,1,1)=vobj(v1,1,1)_^MSGEXC(vobj(v1,-3),vobj(v1,-4),vobj(v1,-5),von)
