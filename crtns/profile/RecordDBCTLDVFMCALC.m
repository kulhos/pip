 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordDBCTLDVFMCALC ****
 ; 
 ; 02/24/2010 18:39 - pip
 ; 
 ;
 ; Record Class code for table DBCTLDVFMCALC
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBCTLDVFMCALC",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
 S vobj(vOid,-4)=""
...
vRCgetRecord0(v1,v2,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBCTLDVFMCALC"
 S vobj(vOid)=$G(^DBCTL("SYS","*DVFM",v1,"D",v2))
 I vobj(vOid)="",'($D(^DBCTL("SYS","*DVFM",v1,"D",v2))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBCTLDVFMCALC",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 S vobj(vOid,-4)=v2
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
...
vRCgetRecord0Opt(v1,v2,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S dbctldvfmcalc=$G(^DBCTL("SYS","*DVFM",v1,"D",v2))
 I dbctldvfmcalc="",'($D(^DBCTL("SYS","*DVFM",v1,"D",v2))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,DBCTLDVFMCALC",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q dbctldvfmcalc
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","CNTRY"))&($P($E($G(vobj(this,-100,"1*","CNTRY")),5,9999),$C(124))'=vobj(this,-3)))!($D(vobj(this,-100,"2*","TYP"))&($P($E($G(vobj(this,-100,"2*","TYP")),5,9999),$C(124))'=vobj(this,-4))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) Q:'($D(^DBCTL("SYS","*DVFM",V1,"D",V2))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,dbctldvfmcalc,vkey1,vkey2) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("CNTRY,DES,TYP",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N dbctldvfmcalc S dbctldvfmcalc=$$vRCgetRecord0^RecordDBCTLDVFMCALC(CNTRY,TYP,0)
 .	S vobj(dbctldvfmcalc,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBCTLDVFMCALC(dbctldvfmcalc,$$initPar^UCUTILN(params)) K vobj(dbctldvfmcalc,-100) S vobj(dbctldvfmcalc,-2)=1 TC:vTp  
 .	K vobj(+$G(dbctldvfmcalc)) Q 
 Q 
 ;
vhasLiterals() ; 
 Q 0
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I '(''($D(^DBCTL("SYS","*DVFM",V1,"D",V2))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^DBCTL("SYS","*DVFM",vobj(this,-3),"D",vobj(this,-4))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
vRCdelete(this,vRCparams,vRCaudit,isKeyChange) ; 
 ...
 ;  #ACCEPT DATE=04/22/04; PGM=Dan Russell; CR=20602; Group=BYPASS
 ;*** Start of code by-passed by compiler
 ZWI ^DBCTL("SYS","*DVFM",vobj(this,-3),"D",vobj(this,-4))
 ;*** End of code by-passed by compiler ***
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBCTLDVFMCALC","MSG",1767,"DBCTLDVFMCALC."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I (vobj(this,-3)'=$ZCONVERT(vobj(this,-3),"U")) D vRCvalidateDDerr("CNTRY",$$^MSG(1476))
 ...
 I ($L(vobj(this,-4))>2) D vRCvalidateDDerr("TYP",$$^MSG(1076,2))
...
vRCvalidateDD1(this) ; 
 ...
 I (vobj(this,-4)'=$ZCONVERT(vobj(this,-4),"U")) D vRCvalidateDDerr("TYP",$$^MSG(1476))
 I ($D(vobj(this,-100,"2*","TYP"))&($P($E($G(vobj(this,-100,"2*","TYP")),5,9999),$C(124))'=vobj(this,-4))) I ($L(vobj(this,-4))>2) D vRCvalidateDDerr("TYP",$$^MSG(1076,2))
...
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("DBCTLDVFMCALC","MSG",979,"DBCTLDVFMCALC."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey2 S oldKey2=$S($D(vobj(this,-100,"2*","TYP")):$P($E(vobj(this,-100,"2*","TYP"),5,9999),$C(124)),1:vobj(this,-4))
  N V1,V2 S V1=vobj(this,-3),V2=vobj(this,-4) I ($D(^DBCTL("SYS","*DVFM",V1,"D",V2))#2) D throwError($$^MSG(2327))
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordDBCTLDVFMCALC(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("DBCTLDVFMCALC",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
  S vobj(this,-4)=oldKey2
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordDBCTLDVFMCALC.copy: DBCTLDVFMCALC
 ;
 Q $$copy^UCGMR(this)
