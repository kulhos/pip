 ; 
 ; **** Routine compiled from DATA-QWIK Filer RecordCTBLFAP ****
 ; 
 ; 02/24/2010 18:38 - pip
 ; 
 ;
 ; Record Class code for table CTBLFAP
 ;
...
vcdmNew() ; 
 ...
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordCTBLFAP",vobj(vOid,-2)=0,vobj(vOid)=""
 S vobj(vOid,-3)=""
...
vRCgetRecord0(v1,vfromDbSet) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordCTBLFAP"
 S vobj(vOid)=$G(^CTBL("FAP",v1))
 I vobj(vOid)="",'($D(^CTBL("FAP",v1))#2)
 S vobj(vOid,-2)=1
 I $T K vobj(vOid) S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,CTBLFAP",$EC=",U1001,"
 S vobj(vOid,-3)=v1
 ;*** End of code by-passed by compiler ***
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=SCOPE
 Q vOid
...
vRCgetRecord0Opt(v1,vfromDbSet,v2out) ; 
 ...
 ;  #ACCEPT DATE=02/26/2008; PGM=Dan Russell; CR=30801; Group=BYPASS
 ;*** Start of code by-passed by compiler
 S ctblfap=$G(^CTBL("FAP",v1))
 I ctblfap="",'($D(^CTBL("FAP",v1))#2)
 S v2out=1
 I $T S $ZE="0,"_$ZPOS_",%PSL-E-RECNOFL,,CTBLFAP",$EC=",U1001,"
 ;*** End of code by-passed by compiler ***
 Q ctblfap
 ;
...
vSave(this,vRCparams,vauditLogSeq) ; 
 ...
 .	I ($D(vobj(this,-100,"1*","FAP"))&($P($E($G(vobj(this,-100,"1*","FAP")),5,9999),$C(124))'=vobj(this,-3))) D vRCkeyChanged(this,vRCparams,.vRCaudit) Q 
 ...
 .	Q 
 E  I (%O=3) D
 .	  N V1 S V1=vobj(this,-3) Q:'($D(^CTBL("FAP",V1))#2) 
...
vselectOK(this,userclass) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vselectOptmOK(userclass,ctblfap,vkey1) ; PUBLIC access is allowed, no restrict clause
 Q 1
 ;
vgetLogging() ; 
 Q "0"
...
vlogSelect(statement,using) ; 
 ...
 ;
columnList() ; 
 Q $$vStrRep("DESC,FAP,PGMINP,PGMOUT",",",$char(9),0,0,"")
 ;
columnListBM() ; 
 Q ""
 ;
...
getColumnMap(map) ; 
 ;
 S map(-3)="FAP:T:"
 S map(-1)="DESC:T:1;PGMINP:T:2;PGMOUT:T:3"
 Q 
 ;
vlegacy(processMode,params) ; 
 N vTp
 I (processMode=2) D
 .	N ctblfap S ctblfap=$$vRCgetRecord0^RecordCTBLFAP(FAP,0)
 .	S vobj(ctblfap,-2)=2
 . S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordCTBLFAP(ctblfap,$$initPar^UCUTILN(params)) K vobj(ctblfap,-100) S vobj(ctblfap,-2)=1 TC:vTp  
 .	K vobj(+$G(ctblfap)) Q 
 Q 
 ;
vhasLiterals() ; 
...
vRCmiscValidations(this,vRCparams,processMode) ; 
 I (("/"_vRCparams_"/")["/VALST/")  N V1 S V1=vobj(this,-3) I '(''($D(^CTBL("FAP",V1))#2)=''processMode) D
 .	N errmsg
...
vRCupdateDB(this,processMode,vRCparams,vRCaudit,vRCauditIns) ; 
 ...
 .	;*** Start of code by-passed by compiler
 .	if $D(vobj(this)) S ^CTBL("FAP",vobj(this,-3))=vobj(this)
 .	;*** End of code by-passed by compiler ***
 .	Q 
 Q 
 ;
...
vRCrequiredErr(column) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("CTBLFAP","MSG",1767,"CTBLFAP."_column)
...
vRCvalidateDD(this,processMode) ; 
 ...
 N RM S RM=""
 N errmsg N X
 I ($L(vobj(this,-3))>8) D vRCvalidateDDerr("FAP",$$^MSG(1076,8))
 I ($L($P(vobj(this),$C(124),1))>40) D vRCvalidateDDerr("DESC",$$^MSG(1076,40))
 I ($L($P(vobj(this),$C(124),2))>20) D vRCvalidateDDerr("PGMINP",$$^MSG(1076,20))
 I ($L($P(vobj(this),$C(124),3))>20) D vRCvalidateDDerr("PGMOUT",$$^MSG(1076,20))
 Q 
 ;
vRCvalidateDD1(this) ; 
 N ER S ER=0
 N RM S RM=""
 N errmsg N X
 I ($D(vobj(this,-100,"1*","FAP"))&($P($E($G(vobj(this,-100,"1*","FAP")),5,9999),$C(124))'=vobj(this,-3))) I ($L(vobj(this,-3))>8) D vRCvalidateDDerr("FAP",$$^MSG(1076,8))
 ...
 I ($D(vobj(this,-100,"0*","PGMINP"))&($P($E($G(vobj(this,-100,"0*","PGMINP")),5,9999),$C(124))'=$P(vobj(this),$C(124),2))) I ($L($P(vobj(this),$C(124),2))>20) D vRCvalidateDDerr("PGMINP",$$^MSG(1076,20))
 I ($D(vobj(this,-100,"0*","PGMOUT"))&($P($E($G(vobj(this,-100,"0*","PGMOUT")),5,9999),$C(124))'=$P(vobj(this),$C(124),3))) I ($L($P(vobj(this),$C(124),3))>20) D vRCvalidateDDerr("PGMOUT",$$^MSG(1076,20))
 Q 
 ;
vRCvalidateDDerr(column,errmsg) ; 
 N ER S ER=0
 N RM S RM=""
 D SETERR^DBSEXECU("CTBLFAP","MSG",979,"CTBLFAP."_column_" "_errmsg)
 I ER D throwError($get(RM))
 Q 
 ;
vRCtrimNumber(str) ; 
...
vRCkeyChanged(this,vRCparams,vRCaudit) ; 
 ...
 N oldKey1 S oldKey1=$S($D(vobj(this,-100,"1*","FAP")):$P($E(vobj(this,-100,"1*","FAP"),5,9999),$C(124)),1:vobj(this,-3))
  N V1 S V1=vobj(this,-3) I ($D(^CTBL("FAP",V1))#2) D throwError($$^MSG(2327))
 S newkeys=newKey1
 S oldkeys=oldKey1
 ...
 S vobj(newrec,-2)=0
 S vTp=($TL=0) TS:vTp (vobj):transactionid="CS" D vSave^RecordCTBLFAP(newrec,$$initPar^UCUTILN($$initPar^UCUTILN("/NOVAL/NOCASDEL/NOJOURNAL/NOTRIGBEF/NOTRIGAFT/"))) K vobj(newrec,-100) S vobj(newrec,-2)=1 TC:vTp  
 D
 .	N %O S %O=1
 .	N ER S ER=0
 .	N RM S RM=""
 .	;   #ACCEPT Date=10/24/2008; Pgm=RussellDS; CR=30801; Group=ACCESS
 .	D CASUPD^DBSEXECU("CTBLFAP",oldkeys,newkeys)
 .	I ER D throwError($get(RM))
 .	Q 
  S vobj(this,-3)=oldKey1
...
vStrFnd(object,p1,p2,p3,qt) ; String.find
 ...
 Q p2
 ;
vReCp1(v1) ; RecordCTBLFAP.copy: CTBLFAP
 ;
 Q $$copy^UCGMR(this)
