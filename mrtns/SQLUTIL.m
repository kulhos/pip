...
EXPORT1(vtbl,hdropt)	
	...
	S expr=$$UPPER^%ZFUNC($p(expr,$C(13),1))
	S z=$$UPPER^%ZFUNC(expr)
	I $E(z,1,7)'="SELECT " Q
	...
	.	S hdr=$$UPPER^%ZFUNC(hdr)				; Uppercase
	.	;							; 
...
DDL1(table)	; Create table definition
	...
	.	D SPLIT("ORACLE",table,.tblarr,outfile)
	.	N spltbl S spltbl=""
	...
	;
	E  D  ;Regular Table
	.	I $D(^STBL("RESERVED",table)) S tmptable="S_"_table
	.	E  S tmptable=table
...
index(proftbl)	; Index definition
	...
	.	S key=$$replace(key)
	.       I $D(^STBL("RESERVED",indtbl)) S temptab="S_"_indtbl
	.       E  S temptab=indtbl
	.	;
...
SPLIT(db,tbl,array,file)	
	...
	E  S dir=$$HOME^%TRNLNM()
	;
	D MAP^DBSDDMAP(tbl,.A) S X="" F  S X=$O(A(tbl,X)) Q:X=""  D  
...
TABLE	F  S TABLE=$O(^DBTBL("SYSDEV",1,TABLE)) Q:TABLE=""  DO
	...
	.       D EXPORT(TABLE,file,3,124,,.delflg) 
	.	I 'delflg D
	..	       USE 0
	..        W !,TABLE
