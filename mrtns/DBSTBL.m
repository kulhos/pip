...
MAIN(vREF,vINFO,vPT,vPB)	;
	...
	.	Q 
	;
	S vkeylen=$piece(vINFO("COL",1),$C(9),3)
	;
...
KEYS(vINFO)	; vINFO array
	...
	;
	I '(vINFO("UFK")="") F I=1:1:$L(vINFO("UFK"),",") I '$$vStrLike(OPTS,"%"_$piece(vINFO("UFK"),",",I)_"%","") S OPTS=OPTS_","_$P(vINFO("UFK"),",",I)
	;
...
DSPREC(dispTwo,vINFO,vr)	;
	...
	.	N DISPLAY N format
	.	;
	.	S DISPLAY=$piece(REC,$C(9),COL)
	.	;
	.	S length=$piece(vINFO("COL",COL),$C(9),3)
	.	I length>0 S DISPLAY=$E(DISPLAY,1,length)
	.	;
	.	S format=$piece(vINFO("COL",COL),$C(9),2)
...
OVERFLOW(ptr,num,vINFO,vr)	;
	...
	.	;
	.	S length=$piece(vINFO("COL",I),$C(9),3)
	...
	..		F ptr=ptr:1:num I $L($piece(vr(ptr),$C(9),I))>width S width=999 Q 
	..		Q 
	.	Q 
	;
	Q (width>41)
...
FETCH(vrs,vptr,vnum,vINFO,vr)	;
	...
	...			S vdata=V_$C(9)_$piece(vdata,"|",vINFO("PIECE"))
	...			Q 
	..		Q 
	.	;
	.	Q:(vrptr=0) 
	...
	..		N fmt
	..		;
	..		S fmt=$piece(vINFO("COL",vI),$C(9),2)
	..		;
	..		I '((fmt="T")!(fmt="U")!(fmt="F")) D
	...			;
	...			S dec=$piece(vINFO("COL",vI),$C(9),4)
	...			;
	...			I (fmt="$"),(dec="") S dec=2
	...			;
	...			S $piece(vdata,$C(9),vI)=$$EXT^%ZM($piece(vdata,$C(9),vI),fmt,dec)
	...			Q 
	..		Q 
	.	;
...
PICK(vrs,vINFO,vr,vREF,value)	;
	...
	;
	S vfmt=$piece(vINFO("COL",1),$C(9),2)
	S vkeylen=$piece(vINFO("COL",1),$C(9),3)
	;
	F  D  Q:visDone 
	.	;
...
HLP(vrs,vINFO,vr,vREF)	;
	...
	S desc=$piece(key,$C(9),2) ; key_TAB_desc
	S key=$piece(key,$C(9),1)
	Q:(key="")  ; Invalid
	;
	...
	.	Q 
	;
	S HLP(1)=$piece(vINFO("COL",1),$C(9),5)_" "_key_" - "_desc
	S HLP(2)=""
	S CNT=3
	;
	F  Q:'($$vFetch1())  D
...
FND(vINFO)	; vINFO array
	...
	S VAR(1)=$$^MSG(299)
	;
	F I=1:1:vINFO("COLCNT") S VAR(I+1)=$translate($piece(vINFO("COL",I),$C(9),5)," ","_")
	;
	S OP=$$^DBSMBAR(26,"","","",.VAR)
	D SETMODE
	;
...
DINAM(vREF,vINFO)	;
	...
	.	N DQQRY N XWHERE
	.	;
	.	F I=1:1:$L(vINFO("QUERY"),$C(1)) S DQQRY(I)=$piece(vINFO("QUERY"),$C(1),I)
	.	;
...
PARSE(params,VALCMDS,vINFO)	;
	...
	..		I (EXPR[":") S EXPR=$$vStrRep(EXPR,":"," & ",0,0,"")
	..		;
	..		I ($D(vINFO("QUERY"))#2) S vINFO("QUERY")=vINFO("QUERY")_$C(1)_EXPR
...
LOOKUPPP(FORMAT,FILES,ARRAY)	;
	...
	;
	I TABLE="CIF",$piece(vINFO("COL",1),$C(9),1)'="CIF.ACN" S ER=1 S RM="CIF.ACN"
	E  I TABLE="DEP",$piece(vINFO("COL",1),$C(9),1)'="DEP.CID" S ER=1 S RM="DEP.CID"
	E  I TABLE="LN",$piece(vINFO("COL",1),$C(9),1)'="LN.CID" S ER=1 S RM="LN.CID"
...
vFetch1()	;
	...
	S rs=$S(vos4=$$BYTECHAR^SQLUTL(254):"",1:vos4)_$C(9)_$P(vos5,"|",1)
	;
	Q 1
	;
vtrap1	;	Error trap
