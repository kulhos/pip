SQLFUNCS;;Library	of functions for use with SQL 
	...
	;
	;
	; 05/16/01 - SANTOSL - 44734
	;	     As Per Chris Richards
	;            Modified SQLTOSHR section to tokenize VAL before
	;	     removing all characters after the comma should not
	;	     be removed if the commas are contained within double
	;	     or single quotes.
	;
	; 02/18/99 - BECKERW - 28755
	;            Corrected if statement.
	;	     Changed  "I ER!(cnt)=0" to "I ER!(cnt=0) S".
	;
	; 08/10/98 - SIGDAE
	;            Added SQL scalar functions NVL, DECODE, ROUND and SIGN.
	;
	; 10/21/97 - SPIER - 26483
	;            Convert data item names to remove spaces, some sql commands
	;	     contain spaces between lists of data items.
	;
	; 09/23/97 - SPIER - 25624
	;            In SQLTOSHR section, allow short name to be built for
	;	     any field which is not a key.
	;
	; 03/07/97 - RATMANSKYD - 24145
	;            Added section SQLTOSHR to convert SQL command to file 
	;	     Local Array.
	;            
	;----------------------------------------------------------------------
FETCH(cursor,rownum,row,stat,rowdata,cnt,rows)	;Private;Get next row
	...
	; have called OPEN with /ROWS qualifier prior to first use.
	;
	...
	.	S ER=$$^SQL(SQL,"/ROWS="_rows,.stat,.rowdata,.cnt)
	.	;I ER!(cnt)=0 S DONE=1
